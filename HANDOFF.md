...
2026-09-19 K3·OPENING-NOVEL-CLOSE-01（老板最新决定只收尾长篇开书，覆盖历史"四类型接通"）完成并已发布：提交121540bc已推送，生产wm-v7-20260919-050500-121540bc、静态8a7c1a8397396c1d3e7d（与本地构建逐位一致）、迁移132无新增。作品类型仅长篇小说开放新请求：短篇/自传/剧本展示但真实disabled+"暂未开放"标签、新草稿默认长篇；服务端create/revise/恢复/AI与手动confirm统一409边界（写任务/排队/模型调用/建书前），零新增任务/书籍/模型调用；已完成确认幂等返回原书，历史非长篇任务/候选只读不改型，缺creativeProfile旧客户端兼容novel；旧非长篇草稿保留输入并阻断提交直至作者主动选长篇。修复路由漏传creativeProfile既有BUG。验证：后端3文件25/25（长篇300万HTTP全链+三类型参数化409反例+幂等重放）、前端3文件68/68、api/author-app tsc零错误、测试tsc持83基准、构建+静态校验+RB门禁通过（RB-19~27核对值随源重算）；浏览器390/1440实看禁用态/暂未开放标签/长篇默认/三成员红玉默认/底部自然流不遮挡。发布：备份checksums校验、迁移预演幂等、保护表哈希不变、回滚包预演通过、连续30秒零在途、逐服务健康、公网14资产sha256、双站index字节一致、4受保护路由401。本批未调用真实模型、不测速；既有旧债（runtime-closure 120项、根tsc 83项）照旧零扩大；全部作者数据、评测记录、生成结果与用量保留。开书页顺序/三成员/红玉默认/底部非悬浮维持已验收状态。详细证据在原任务outbox结果文件顶部；老板自行往后测试（设定/资料/时光机），Codex后续只核对本批差异。

2026-09-19 K3·OPENING-UI-02返修（Codex独立验收R1/R2）完成并已发布：提交2861da92已推送，生产wm-v7-20260919-033900-2861da92、静态7438953c19970c436f33、迁移132无新增。①预计总字数按作品类型同一合同（长篇10万—1000万/短篇1千—10万/自传1万—200万/剧本5千—200万）全链穿透，短篇1万字不再被误拒；②非长篇的设定/时光机任务创建入口服务端409门禁（写任务/排队/占预算前），历史任务只读保留。验证：domain 42文件420全绿（新类型合同10+Codex反例4）、前端241、安全/迁移/数据安全全绿、tsc基准不增、RB-19~27核对值更新；发布备份校验、迁移预演幂等、保护表哈希不变、30秒零在途、公网14资产哈希、双站200、4受保护路由401。生产只读核查book_creative_profiles共10行全novel、无非长篇异常任务。本批未调用真实模型，真实效果由老板实测；既有夹具信封兼容修复33项既有失败、迁移清单补0126-0132既有失修均已记录。详细根因与证据在原任务outbox结果文件顶部；交Codex独立验收。
...
2026-09-16 K3·d7fc67f5复核后C反馈调度修复+单节点闸门完成（提交3b87663d已推送，未部署）：①design()改为初稿同版本上自检+独立审查收齐阻塞（审查不再被自检失败跳过），带来源汇总去重（完全相同才合并、suggestions不升级）统一进入唯一一次自动修订；修订版复检仍阻塞诚实revise、无revisionRound=2；全通过不重修；verdict分绑初稿/修订版候选revision（仓储1起）；11/12审查问题未送达的流程缺口关闭。六场景反例（双源/仅自检/仅审查/全通过/仍阻塞/审查技术失败原边界/重入零重复）。②single-dispatch-permit仅隔离探针使用：绑run+step登记attempt+快照模型（resolver装配侧，ModelRequest无modelId字段）+输入hash入档，先消耗再dispatch、持久化重启不重置、其余dispatch网络前不可重试阻止；fake fetch四反例过。③验证：9套件118/118、tsc过。下轮真实验证强制门禁写入25.9（B同轮volume-card:0单dispatch、max_tokens32000、≤600秒、许可文件消耗即停）。老板已批示接续MODEL-NODE-EVAL（原任务顶部），本批完成后启动。不部署。
...
2026-09-16 K3·1f831c6a复核离线收束完成（提交d7fc67f5已推送，未部署）：零真实调用。①参数表回归断言到mock fetch请求体：volume-card 3000/6000/8000×GLM(11000/14000/32000,省略thinking)×DeepSeek(3000disabled/10000e4000/20000e4000)×Kimi(3000disabled/12000e4000)；真实适配器+网关验证reserved=max_tokens+输入字节+2048逐字节相等、4500字提示约47k<64000窗口；如实记录DeepSeek声明预算4000与实际余量12000不匹配，未调任何推理参数。②截断安全统计TruncationDiagnostic（stopReason/可见字符数/推理块有无/分项token未上报null不猜），双fixture区分有无部分正文，正文思维链不落库，known/unknown不变。③C追踪（探针库只读）：12条审查issues中11条从未进入修订输入——自检先用掉唯一自动修订轮（:528），审查在revisionRound=1产出无轮可送（:534），属反馈路由未送达非作者决策；1条（三项核心制度）反馈送达但模型未改到位。最小复验方案（待核定）：B同轮retry单节点volume-card:0（GLM 32000，约1调用/5-10分钟）用新统计实证；A的400根因仍未知（新轮只到卷0不能认定消失）；C修复方向=反馈路由非加轮。25.8过度断言已原位修正、新增25.9。验证：59+70+33+7直跑全过、tsc过。交Codex复核，不部署。
...
2026-09-16 K3·ab8464c4定点恢复实跑完成（提交1f831c6a已推送，未部署）：①A新轮f573938f未复现400——供应商白名单诊断捕获就位，根因保持如实未知；②6000卷卡预算被实跑证伪：DeepSeek在>5000翻转为显式思考+4k预算烧满10000零可见、GLM可见路由6000+8000=14000被隐式思考烧穿，A/B双双截断volume-card:0；同跑skeleton以8000+综合余量成功→卷卡并入8000综合节点组（节点级，不全局调大，可见输出仍由合同校验与6000字符上限封顶），反例同步改断言8000。③预算：本批5调用/55722保守token/约1155秒，端到端累计32/289298/约3507秒——墙钟余量不足再次整轮恢复，8000修正未真实复验（离线反例+tsc通过），待下一预算窗口。④C维持定性：12条issues全真实、修订轮已尽需作者决策；无自然过审候选，采用验证未执行。⑤migration.test.ts 1失败经stash验证为09aa19e4前既有旧债，与本次无关。恢复证据在.local/dispatch/recovery/（result.json+probe-recovery.sqlite），原证据D:/tmp只读未动。详见outbox/task-rebuild-closeout-01-s1.result.md新小节。交Codex复核，不部署。
...
2026-09-16 K3·最终候选验证完成（冻结ab8464c4已推送，未切换生产）：①独立归档sha256 097aeb57…（依赖全在归档内，Node24.19+npm11.19.1未绕engines）全量构建+静态manifest验证通过；0126迁移幂等零新增；旧版回退在已迁移库上零新增放行+旧API健康读取正常。②真实通道探针7/7（6模型Messages路径+glm-5.3 opening_design专用Chat路径，合成输入，用量全已知）。③端到端一轮（27调用/233,576 committed tokens/39分钟，预算100/60万/3600秒内有富余）：推荐✓→确认✓→C审查revise（诚实未过审）、A卷卡3遇http-400、B卷卡0截断（glm输出11,000触上限）→无可采用未执行HTTP采用，产物与隔离库留存D:\tmp\wenmi-s1a-probe-final。**不满足自然过审+采用放行条件**；A的400与B的截断为下一批定点收束节点。仍保留：数据库边界恰原7项、闭包69旧债、verify:full其余旧债、CTX未做。RB-21代码核对值已随本批registry改动同步（ab8464c4）。探针脚本一处平台适配已声明（/tmp门禁1行，被测产品代码为冻结dist未改）。详见outbox/task-rebuild-closeout-01-s1.result.md顶部小节。交Codex一次性核定，不部署。
...
2026-09-16 K3·18f3c2a0复核后Agent Plan通道收尾完成（提交9523942a已推送，分支codex/auth-takeover-01-release，未部署）：按授权接手指定Codex Agent Plan未提交文件，全程Node v24.19.0。15项foundation适配完成（72/72含4新反例：退役覆盖变量、旧Coding仅解码冻结任务、仅按量密钥不调封面、失败不回退）。关键环境修正：node_modules/@wenmi/v7-backend与opening-runtime的JUNCTION原指向主区陈旧dist掩盖漂移，已重定向worktree自身并重建dist（其余junction未动，time-machine-core含Codex未提交execution.ts受保护）。暴露漂移逐项定性修复：cutover机械对齐（opening-platform 3处、prompt-compiler 1处、manifest-compiler）、名册漂移（opening-ranking按5079844a新合同改六人阵容+GLM进设定编选；creation-pipeline规划成员3→4+memberPreferences恢复GLM接手场景）、闭包登记connectivity脚本（回69旧债基线）。超清单判断两处请Codex追认：legacy-opening四文件校验器coding→agent（工厂已拒coding，不翻则legacy开书流必坏）+junction重定向。最终验证（全实跑Node24.19）：foundation+cutover 76、S1材料选择+集成修改件 120、backend vitest 102、node-test回归 16、受影响第二批 225、API tsc过、双dist重建；闭包69旧债、数据库边界恰原7项未过（未放宽）。仍阻塞：真实Messages/长任务过审、CTX未证明、通道生产验证、正式发布未放行。详见outbox/task-rebuild-closeout-01-s1.result.md顶部小节。
...
2026-09-16 K3·8caac9a5复核后边界修复+本机预演完成（提交18f3c2a0，分支codex/auth-takeover-01-release，未发布）：①边界修复——StorylineMaterialRepository新增runInTransaction同步回调（BEGIN IMMEDIATE/单次COMMIT/异常先ROLLBACK/嵌套并入外层），storyline-material-service save()整体走该回调，应用层.prepare/.exec已清零（grep无匹配）；材料/选择集成20/20、API tsc通过；边界测试仍失败但违规恰为原7项逐名（新增项已消除，未删门禁未改白名单）。②本机预演（.local/dispatch/release-rehearsal/，candidate=18f3c2a0/old=a7614958）——git archive干净构建+contracts/v7-backend/api/worker+static-release构建与verify全过（releaseId ffa140770069b0ff40db；环境偏差：本机node v24.15.0<要求24.16.0，用--engine-strict=false，已记录）；空库0001→0126全应用、重复零新增；**回退实证：旧代码迁移器缺0126文件fail-closed拒绝启动，回退包必须保留新迁移文件**（old树中该文件系验证手工复制非自带），补入后旧代码读写冒烟正常（needs_redesign默认0透明）。③清单更正——inbox任务"K3·S1-A发布准备清单"已原位更新（待发布45提交/25触代码、run5与CTX措辞更正、§5基准10bcf7e5+18f3c2a0+回退保留迁移文件+旧标签页零任务、新增§7预演记录）；s1.result.md顶部新增对应小节并同步更正旧小节两处过时表述。仍阻塞：通道恢复+Codex核定最终批次（run5风险单列）+发布窗口服务器只读复核现网releaseId。交Codex复核，不部署。
...
2026-09-16 K3·S1-A发布准备清单完成（10bcf7e5基准，仅准备不部署）：现网wm-v7-20260914-151504-a7614958（9d3d26b5记录，releaseId待发布窗口服务器只读复核）；待发布44提交/24触代码，增量迁移仅0126纯加法；现网前端旧设计合同被新后端拒绝→API/Worker/静态必须同窗一批；Coding Plan已到期，S1-A功能依赖模型调用→Agent Plan通道恢复（Codex未提交差异，不冻结不包含）必须先于或同批且有独立验证；结论=代码包具备候选构建条件、整批发布尚不可执行，最小剩余=通道恢复+Codex核定批次与boundary新增项+服务器只读复核现网版本。清单全文在inbox/task-rebuild-closeout-01.md"K3·S1-A发布准备清单"，结论在outbox/s1.result.md对应小节。交Codex集中核定发布批次。
...
2026-09-16 K3·72c3a62f复核后六项离线修复完成（隔离worktree分支codex/auth-takeover-01-release，基线72c3a62f，未提交未发布；全程未真实模型调用、未碰生产）：①故事线资料自含selectedLines正文，资料页标题/描述直接可编辑（稳定id、role服务端裁定、推荐原件不变、旧版本服务端回填）；②design-runs带expectedMaterialRevision，版本号或内容哈希不符均409零写入，旧选择不得静默盖新资料；③影响预览返回版本签名、确认保存事务内重算不符409带新预览零写入；④planning-context命中needs_redesign已采用基线→409，预览区分全书卷概要真实计数与未实现独立卷设计；⑤审查相位member=实际reviewer、卷卡相位=writer、进行中统一"正在工作"、已持久化任务显示可离开说明；⑥CTX措辞按run-log实际分母修正（有效提取3/7遗漏、方案3/7术语未命中、因果6/7完整+1份稀释、中断用量未知不冒充零）、run5时限3600秒→150分钟依据登记（run4约45分钟到期方案仍排队+run5实测95.3分钟）、verify:full旧债逐项实跑更新（rebuild-control已修12/12、creative-reference-migration现通过、其余8项台账/迁移期望/创作链夹具债原样、测试typecheck37处本批零新增、test:full未重跑整体仍标未通过）。验证：材料/选择集成19/19、阶段一回归30/30、解析12/12、页面21/21（新增4用例）、API tsc、author-app tsc+vite build、真实Edge三宽度截图8张含工作态/编辑态（outbox/s1a-browser-evidence/stage3/，harness tests/browser/s1a-storyline-material-browser.test.ts门控S1A_BROWSER_STAGE3=1）。文档：TIMEMACHINE_STORY_DESIGN 25.1/25.2/25.3更新+新增25.6、执行计划RB-22加一条（CTX分母措辞同步修正）。报告outbox/task-rebuild-closeout-01-s1.result.md顶部"K3·72c3a62f复核后六项离线修复"节。**受阻**：真实模型与CTX阶段二待老板续费；整包不发布待Codex复核。

2026-09-16 Agent通道补验：八个文字模型DeepSeek Pro/Flash、GLM5.3/Flash、Kimi2.7/K3、豆包、MiniMax均通过当前环境Agent凭据的小型HTTP200探针；GLM必须enabled thinking，disabled反例400后纠正。仅连通性，不是长篇质量；未核对是否为老板新贴密钥、未替换生产环境。CTX runner也改Agent并阻止混用旧证据目录，部署模板同步。Codex配置/名册/模型文件及测试脚本需与K3并行资料页差异分开核对，未整包发布。

2026-09-16 Codex并行完成Agent Plan通道代码调整（老板已授权所有模型统一Agent Plan）：改动仅models/model-runtime-config.ts、model-adapter-factory.ts、ark-plan-model.ts、volcengine-ark-image-gateway.ts，以及backend的agent-governance-registry/agent-roster/character-memory-runtime/creation-runtime/planning-editorial-runtime。新增tests/unit/agent-plan-cutover.test.ts和无密钥的scripts/quality/agent-plan-connectivity.mjs。配置4/4、三套S1回归28/28、backend build和API typecheck通过；真实连通性未确认，未写生产密钥/未部署，勿称已恢复线上。旧Coding快照保持可读但拒绝新调用，不静默重绑；新名册/默认角色全Agent，缺Agent凭据明确拒绝，不回退按量封面。K3资料页并行修改全部保留，不混合提交；接续时核对本组diff并纳入独立冻结验证。用户给的新密钥只能安全配置运行环境，不写入工单/Git/日志。

2026-09-16 Codex验收更正：下方“三阶段全部完成”不成立，整包不发布。材料/解析独立21/21通过，但资料正文不可编辑、旧选择可反向覆盖新材料、影响预览缺下游版本签名、旧卷上下文未拦失效基线、工作UI未兑现。K3先执行主区 D:/wenmixiezuo/.local/dispatch/inbox/task-rebuild-closeout-01.md 顶部“72c3a62f复核后K3接续”的离线修复；原s1.result.md有详细证据。run5未采用/CTX部分受阻，暂停新模型调用，保留已有成果，不部署。

2026-09-16 REBUILD-CLOSEOUT-01 S1-A三阶段全部完成（HEAD=d8c7c1a9，分支codex/auth-takeover-01-release，未发布）：①阶段一P1-1～P1-6修复+真实探针run5——方案C全步骤成功但两轮审查verdict=revise未过审（伙伴线/卷D收束/锚点缺口，审查按设计拦截），A/B卡glm-5.3思考烧穿截断（已知失败），45调用已知406,680+未知预留26,066 tokens/95.3分钟<150分钟期限，三分计量全程有效；**未达成自然过审+采用，不伪造不重跑**；②阶段二故事线资料页（提交e67f2fe8）——二级导航+资料编辑/草稿/失效机制（迁移0126），集成9/9+回归28/28+页面17/17+真实Edge三宽度8截图；③阶段三CTX-01资料长度实测（工具scripts/quality/ctx01/，提交e37408e8/d5b4f194/ae017f73）——18次计划=14成功+4套餐到期受阻，15k/20k档提取3/9遗漏核心设定整句、方案7/7因果合规，**15000字符规范未通过实测验证为安全默认，不上调长度承诺**，优先排序前置/去重/分包/生成后对照/独立审查。收尾修复d8c7c1a9：541dd570引入的上线列非法取值致rebuild-control严格解析必503（后台功能地图），已登记"已发布(后端)/已发布(后端部分)"白名单+图例释义+RB-02验收回退未验证（前端未开始），测试12/12含新反例。**新阻塞**：火山方舟Coding Plan套餐2026-09-16 00:29到期（400 InvalidSubscription），CTX-01阶段二（多题材/多模型/重复，样本已备妥、runner断点续传）与一切后续真实模型调用待老板续费决策；A/B失败轮是否续做同待决策。报告outbox/task-rebuild-closeout-01-s1.result.md（顶部K3阶段一收束/阶段二/阶段三CTX-01/收尾四节）；证据outbox/s1a-browser-evidence/与outbox/ctx01-evidence/。本批未发布，发布走AGENTS.md第7节既有授权。

2026-09-15 S1-A“30a6f053复核”定点修复完成（提交256db4e5+b91ff23e，HEAD=b91ff23e，未发布）：①探针计量与退出（s1a-probe-budget.mjs纯逻辑+6/6单测）：成功与knownUsage失败都累计、未知单列不按0计、发起前按在途预留、全终态失败立即结束逐轮写状态，预算100次/600k tokens/3600秒未扩大；②长度截断机器可读分类（ModelAdapterError.causeCode→网关truncated，不再temporary盲重试，HTTP400/unknown分型保留）；③卷卡逐卷有界生成（快照volumeStrategy=per-volume-v1，单卷请求+硬预算提示词，截断同轮retry只重做被截断卷，StepRepository.retryRunFailed新方法）；④suggestedVolumes显示名系统映射+不可映射精确修复+逐卷预检（含run3真实根因keywords>12归一化、整卡6000字符守卫）；⑤生产选择函数排除kimi-k3（A红玉deepseek/B幼薇glm/C陆青禾doubao，探针log可证），每方案异模型reviewer。验证：s1a-30a6-fixes 6/6+probe-budget 6/6+定向9套件55/55+部门50/50+页面13/13+双端typecheck。真实探针两次（同60万字样本）：run2三方案如实分类失败（据此修keywords/提示词根因）；run3方案C豆包完整走通逐卷→自检→异模型审查→修订轮（succeeded但verdict=revise未过审），A/B为可恢复truncated；255,419已知tokens/31调用/0未知/0现金；**无方案过审→HTTP采用未达成→按停止条件不发布不机械重跑**；探针自身cookie作用域bug已修（run3暴露）。证据outbox/s1a-browser-evidence/probe-run3.*；报告s1.result.md“30a6f053定点修复交付”节。唯一下一步：Codex决定用已修复探针再跑一次或先调审查阶段输出策略，再发布批收束。

2026-09-15 S1-A“d5e4b0d1复核”发布前验证收束完成（未发布）：①真实交互——复用浏览器harness，诊断并记录工具缺陷（IAB定位器click挂起×2、CUA键盘乱序、截图初次超时），改用fill+CUA真实坐标点击完成登录/勾选/添加/确认/返回/导航全序列，390/800/1440三宽度截图4张已取得并经视觉模型逐张核对（无错位/溢出/截断），证据存outbox/s1a-browser-evidence/*.png；②证据口径——F-gate有回退POST证明持久化准入非自动交接（自动交接依据无回退的浏览器harness）；BREAK_FLAG=请求未完成恢复，“已建轮丢回执”由页面测试单独证明；③真实模型探针（scripts/quality/s1a-http-real-probe.mjs新增）——服务器/tmp隔离目录+独立SQLite+部署env密钥仅进程内，预算100次/600k tokens硬停+1小时墙钟，实际13次/50900 tokens/0元：真实推荐（6条故事线+服务端hash/版本）与结构化确认建轮A/B/C全部通过HTTP当前实现验证；三方案生成失败（volcengine-coding-plan输出上限12000截断卷卡JSON为主因、一次http-400、一次ID合同违反），机制层重试/隔离/诚实失败全部正确，**HTTP采用未验证**——真实端点配置是发布真实模型操作的现实阻碍，交Codex定方向，未重跑；④verify:full精确清单自既有日志提取（13运行时失败逐项+类型10处全在未触碰文件，本批零新增、边界项已减一），未重跑全量、未改白名单。报告outbox/task-rebuild-closeout-01-s1.result.md“d5e4b0d1发布前验证收束”节。

2026-09-15 S1-A“3a84dc98复核”两项补齐+浏览器验证完成（提交72bd24e1+4459b76b，未发布）：①真实账号接线——页面未决记录键改用useAuthorAccount()的已验证userId（AuthorAccountBoundary），删除无业务写入方的localStorage['wenmi:session-owner']读取；账号/书籍变化重置恢复/dirty/未决/输入，账号未就绪不标记恢复完成；13项页面测试改真实SessionProvider渲染+新增账号切换不读旧记录（13/13）。②持久化门禁证据——新增F-gate：部门夹具抽到tests/integration/helpers/setting-department-fixtures.ts（部门套件50/50不回归），实际HTTP推进证明批次未完/条目未确认/总清单未完分别409零设计轮，真实最终整理完成后同idempotencyKey合法请求创建A/B/C并幂等回放，版本=服务端finalReviewRequestHash（无mock门禁、无pv字符串）。③隔离浏览器390/800/1440——tests/browser/s1a-browser-harness.test.ts（环境门控，test:full下skip）：真实登录会话+真实API/vite dev+连接重置模拟未知响应；验证编辑35秒不被轮询覆盖、未决记录按真实userId隔离、刷新后原键自动重试（roundKey与记录key逐字相等、恰好一轮全部succeeded、记录确认后清除）、保存选择刷新恢复、三宽度无水平溢出。验证：s1a-fixes 7/7+页面13/13+部门50/50+定向43/44（边界门禁基线7项按名）。报告outbox/task-rebuild-closeout-01-s1.result.md“3a84dc98两项补齐交付”节。

2026-09-15 S1-A六项集中修复完成（提交d5825f07+e68d7073，未发布）：按90328d31核查逐项修——①快照一致性：startDesignRound事务内两遍构建（空intent探针仅校验上游签名→最终intent同事务重建完整快照），manifest哈希/documents正文/保存文本互相印证，并新增真实HTTP链测试（推荐→结构化design-runs→路由自带执行器生成审查→HTTP adoptions，采用路由以保存intent重建manifest，采用后书籍manifest intent hash=digest(intent)）；②事务门禁与回放：就绪/版本读取移入BEGIN IMMEDIATE新轮分支经路由注入的prerequisiteReader，e68d7073删除客户端版本参数（无读取函数fail-closed拒绝），同键规范请求归属核查后直接回放不重新就绪，第二方案故障整轮回滚；③页面恢复只做一次：bookId+roundKey标记+authorDirty后不重灌+旧推荐ID不套新推荐，双后台刷新保留编辑（页面回归）；④跨刷新防重：完整selection+key按账号+书籍sessionStorage隔离持久化，刷新回填输入并原键自动重试一次，roundKey出现即清除，4xx明确拒绝终结/网络未知保留（按AuthorApiError.status+retryable区分）；⑤authorNote非字符串拒绝；⑥storyline-selection DB访问提取到infrastructure仓储，边界门禁按名回到基线7项。验证：定向7套件42/42+页面12/12+双端typecheck+边界按名对比；verify:full基线债不变未重复执行。真实模型探针（密钥环境不可用已记录）与浏览器三宽度列为发布批前置。报告outbox/task-rebuild-closeout-01-s1.result.md顶部90328d31节，交Codex集中验收；下一步Codex核查d5825f07+e68d7073差异并安排前后端协调发布。

2026-09-15 S1-A完成（提交e58856a1，未发布）：结构化故事线确认与基线启动——design-runs请求合同改为{idempotencyKey,selection{推荐runId/服务端hash/设定版本/勾选/自添/shape/ensemble/authorNote}}，旧intent-only请求400提示刷新且零任务；startDesignRound单事务（来源读取→推荐五重校验→快照→A/B/C创建→COMMIT），幂等按round_key+规范requestHash（同键同选择返原轮/同键不同选择409/无selection元数据拒猜），start()收窄拒绝design；state投影服务端recommendationHash+最小selection供刷新恢复；前端结构化发送+键冻结按选择签名+恢复已保存选择不重置recommended默认。请求边界30/20/80/500/1000/4000字符精确提示不截断；intent服务端从验证后选择重建。验证：定向6套件36/36+页面9/9+双端typecheck+capabilities全绿；verify:full受阻于基线既有债（runtime-closure/台账68/迁移期望等13类，stash基线对照同样失败，如实记录）。真实模型探针与浏览器三宽度列为发布批前置。报告outbox/task-rebuild-closeout-01-s1.result.md，交Codex集中验收；下一步Codex核查差异并安排前后端协调发布。

2026-09-15 S0-revision+S1阶段一完成（HEAD e2eb71da，未部署未生产VACUUM）：①Codex核查5项全修入vacuum-maintenance v3(6323842d…，54f249a6)——失败/信号保留保护快照并打印路径+独占创建+wenmi实测写权限；恢复真实验证(health JSON解析releaseId必须等于期望且status=ok、start/is-active返回码、VACUUM失败恢复前重新核验)；stop返回码+双服务inactive确认+fuser rc>1拒绝；指纹枚举实际业务表(缺表可选/读错失败)+sqlite_sequence+FTS行数+不比物理rowid；生产锁固定不接受注入；测试s0r 28/28+原25项回归25/25。状态纠正采纳：S0.4待03:00 cron到点核对(当前00:57未到)；主区与隔离分支未统一(记录)；S1范围=RB-19/20/21/22。②S1阶段一：盘点确认前置链后端已完整存在(prerequisite三道持久化门禁/handoff幂等派工/recommend→design A/B/C→revisions CAS→adoption编号映射)+既有75项测试覆盖；新增HTTP边界权威性测试(e2eb71da)：未确认时recommendation/design-runs 409消息指向设定链、就绪后同payload进400证明门禁权威；验证：新测试1/1+S1链路4套件28/28+设定部门50/50全绿。S1剩余：真实模型探针(需密钥环境)+浏览器390/800/1440+15000字符口径对最新卡片复核。报告outbox/task-rebuild-closeout-01-s0-revision.result.md，交Codex。
