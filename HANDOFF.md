...
## 最近完成的改动（最新在最上）
1. 卷规划前置门禁恢复（DEC-CURRENT-092）：普通作者尚未完成设定定稿时，卷页不再显示一个注定失败的“开始规划第一卷”；改为读取最新设定质检状态，显示需要作者决定的硬问题数量，并提供“回到设定处理”直接切页。后端设定基线门禁保持不变，不替作者自动确认矛盾。手机端卡片单列、按钮满宽。卷规划定向4项、Web与测试TypeScript、全量184文件779项均通过，生产Web构建产物为`index-C-xTJZNe.js`/`index-BRX8Z8pC.css`。
