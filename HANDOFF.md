...
> 分层创作后续开发的唯一实施与验收总表：`docs/LAYERED_CREATION_IMPLEMENTATION_AND_ACCEPTANCE.md`。部分完成不得勾选完成，未通过E3/E4不得宣布整套方案完成。


## 项目现状（2026-08-21 交接给 Codex）
...
- 当前本地基线：**全量测试 779 绿（184 文件）**，contracts/API/Worker/Web/测试五组 TypeScript 检查、四端生产构建及15文件27项迁移/升级/恢复专项通过，最新迁移仍为 `0062_setting_gap_status.sql`。
...
## 最近完成的改动（最新在最上）
1. 卷规划前置门禁恢复（DEC-CURRENT-092）：普通作者尚未完成设定定稿时，卷页不再显示一个注定失败的“开始规划第一卷”；改为读取最新设定质检状态，显示需要作者决定的硬问题数量，并提供“回到设定处理”直接切页。后端设定基线门禁保持不变，不替作者自动确认矛盾。手机端卡片单列、按钮满宽。卷规划定向4项、Web与测试TypeScript、全量184文件779项均通过，生产Web构建产物为`index-C-xTJZNe.js`/`index-BRX8Z8pC.css`。 生产已采用“新增哈希资源＋原子替换首页”无重启发布：旧JS/CSS继续保留给已打开页面，回滚首页在`/opt/wenmi-releases/0c6e32f-web/index.before.html`；公网首页/新资源200、登录门禁401，API/Worker/Caddy均active，Web与设定/建卷API、Worker哈希和本地一致。
1. 《阵骨问天》生产20章验收进行中（DEC-CURRENT-085）：隔离账号、唯一验收书和14人团队已核对，四项核心设定完成三席独立方案、主编融合与作者确认。整份设定质检在旧生产API上连续3次稳定失败；受限管理员审计确认模型调用成功且零现金回退，但通用`answer`输出合同覆盖了专用`verdict/summary/issues`合同，旧管线又未做该工件的结构预检与同任务纠错。当前API修复已加入专用提示、结构校验和纠错提示，19项定向测试及184文件774项完整verify、五组类型检查和四端生产构建通过；老板已授权仅部署API，须在公网不可见发布目录构建、连续30秒零在途任务后切换，Web/Worker/迁移均不动并保留旧API回滚。部署后只用第4次新质检继续同一本书20章；验收账号临时5000万算力值/48小时/现金回退0，完成后必须恢复青铜。本轮只建立单书20章E4阶段证据，不代替五题材、50/100/200章或手机真机验收。
1. 分层创作工程补缺（DEC-CURRENT-084）：普通作者JSON、失败响应和SSE增加`clean-v1`网络洁净投影，管理员任务审计走独立物理路由；全部TSX统一作者错误门禁并静态禁止`Error.message`/路径/堆栈/Worker泄漏。手机端用`visualViewport`动态高度处理输入法并滚入焦点。十类停止条件和分层写入只读回滚通过集成快照，作者输入、定稿正文、结算、规划版本和调用证据不变。管理员隔离HTTP全链由15步扩展到19步，已从开书走到第一章2876有效字、6份独立审查、作者确认后canon/settled，证据在`data/verification/layered-admin-full-chain-fullchain3.json`。本地184文件773项全绿、五组类型检查、四端构建及迁移/升级/恢复专项通过。E3五题材、I09—I18真实文学效果、E4长期质量和生产手机实机仍未完成；内置浏览器受Windows沙箱`helper_unknown_error`阻断，且本机没有可用真实模型凭证，不能伪造通过。当前改动尚待安全生产部署复验。
1. 分层创作E2工程闭环（DEC-CURRENT-081）：四项核心设定、按需补设定三选一、硬/软/留白上下文、全书故事总线与线程账本、卷方向/事件链拆分、22种隐藏结构方法、双路线整选/片选/融合、首卷前500字/黄金三章/10万字高潮追踪、完整事件章链与近期细纲已经贯通。管理员隔离HTTP冒烟从开书走到第一章详细章纲冻结共15项通过，并据此发现并修复Worker漏注册`event_chain_generation`的真实阻断；证据在`data/verification/layered-admin-planning-final4.json`。360/390/430px组件布局与44px触控门禁通过；浏览器像素级实机复核仍待可用浏览器环境。E3真实模型多题材盲审和E4 20/50/100/200章长期质量未开始，不能宣称整套文学质量验收完成。提交`1144273`已推送，生产Web、API、Worker及0058—0062迁移已上线。生产日志随后暴露讨论取消时阶段状态误写`cancelled`的旧BUG；热修`f9aa664`已在公网不可见暂存目录构建，并由静默守卫确认在途任务连续30秒为0、切换前再次复核后于22:20:41上线。生产运行文件与本地762项全量测试构建哈希一致；双服务active、首页/健康正常、未登录会话401、SQLite完整性与外键检查通过，热修后日志未再出现CHECK错误（DEC-CURRENT-083）。
...
- 已完成（E2工程）：内测说明页、书籍列表页、青黛新中式全局风格、开书向导、开书信息页、创作团队14人、三审+按需找茬、四项核心设定与按需补设定、分层卷方向/事件链/事件/章纲、首卷强启动合同、隐藏结构双路线、任务自愈、标签库、管理后台、会员与个人中心。
- 进行中/下一步：先完成《阵骨问天》同书20章生产验收与账号青铜恢复；当前整份设定质检API修复待按仅API、30秒零任务、旧构建可回滚门禁上线。之后补浏览器可用环境后的360/390/430px真实像素复核，再执行E3五题材和后续50/100/200章长期验收。
