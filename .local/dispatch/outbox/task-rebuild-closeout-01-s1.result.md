...
代码dc543268已推送；API/Worker wm-v7-20260918-173000-dc543268、静态95018c6b23d32a73e653已上线。页面28/28、方案/材料/选择26/26、双端tsc；服务器LF归档干净全链构建、静态14文件验证和方案6/6通过。迁移目录与现网逐字相同，仍132；正式备份20260918T093404Z-590810完整校验，连续30秒零在途后切换。公网14文件sha256、双站入口、目标深链接、匿名401、旧哈希兼容、主编/成员图片200通过。新API PID591625、Worker591648，health ok/ready，切换后错误日志无记录；以wenmi身份只读实际书状态三方案均返回真实chief/assignedMember/reconfirm。

过程纠正：服务器测试最初因准备脚本的RELEASE_ID格式不合规而失败，修成数字时间后6/6；首次切换引用旧代码包中不存在的production-env.py，停服后未切换，先恢复原版，再改用已验证候选工具并加入存在性预检，第二次成功。无迁移/作者数据恢复/新模型调用。浏览器工具已可读取登录页，但会话未登录，登录后三宽度视觉未验，不冒称验收。证据保留worktree `.local/dispatch-evidence/ui-closeout-20260918/` 与服务器候选目录（脚本/备份记录/identity-projection/health/deployment-passed）。自然过审与文学质量缺口不因本UI修复关闭。

## Codex · 时光机残留界面替换已上线（2026-09-18 17:16）

用户截图的页面为当前唯一tm2入口，旧生产TimeMachinePage已删除，残留问题在TimeMachineDirectionPage的进度布局。直接删除独立旧导航CSS、空头像占位、失败“待接手”和下划线“续做”；复用创作页workspace-secondary-tabs；排队不冒充成员工作，工作中显示真实成员和不定进度（不编造百分比），失败显示服务端原因/继续设计，过期资料转确认重设。已确认故事线和要求折叠保留。账户、资料版本、未决恢复、采用门禁全保留。

代码3b3ec501已推送；静态ab04cb4ed9e568dea11e已原子上线。页面27/27、author tsc、作者/后台构建、清单14文件校验、公网14文件逐项sha256、时光机深链接200及新JS、匿名state401均通过。API/Worker PID仍588585/588606，后端e1310473健康，未重启/调用模型/改数据。发布首次wenmi权限不足发生在切换前，核对root所有权后以root执行原切换脚本完成，未改权限。浏览器DOM/截图工具连续超时，390/800/1440视觉未验证。证据：worktree `.local/dispatch-evidence/ui-closeout-20260918/`，含冻结LF源码、静态包、执行脚本和public-result.json。旧哈希资源仅供已打开标签页兼容/回滚，当前入口指向新静态。

范围说明：独立rebuild/apps/author-web仍是未整站切换的重构代码，不属于此生产旧页；未删除其他人的有效研发成果。文学质量缺口仍按前节单列，不因本次UI上线关闭。
