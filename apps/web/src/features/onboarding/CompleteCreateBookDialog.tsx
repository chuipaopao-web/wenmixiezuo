      ...
      <div className="complete-create-book-body" ref={bodyRef}>
        ...
        {step === 1 && !editing && <section className="opening-form-section opening-idea-section">
          ...
          {openingDesignStatus === 'working' ? <div className="opening-ai-wait" role="status" aria-live="polite" aria-busy="true">
            ...
            <label htmlFor="opening-idea">说说您想写什么
              <ImeTextarea id="opening-idea" aria-label="开书思路" maxChars={800} rows={6} value={openingIdea} onChange={(next) => { setOpeningIdea(next); setOpeningDesignError(null); }} placeholder="例如：林砚穿越到仙侠世界，成了青云宗最底层的杂役弟子，资质被判定为废品，每日只能挑水劈柴。三个月后外门大比将清退所有不合格杂役，而他意外发现自己能从废弃法器中提取前任主人的修炼感悟。与此同时，宗门灵脉正在枯竭，失踪多年的掌教留下的禁地令牌却出现在他手中……" autoFocus />
            </label>
            <p className="opening-idea-help">建议把人物、时代、核心脑洞或冲突说具体一些；短想法也可以提交，至少4个字，最多800字。当前 {openingIdea.length}/800</p>
        ...
        <div className="opening-primary-stack">
          ...
          {step === 2 && <section className="opening-form-section subject-section">
            <div className="section-heading"><div><span>02</span><h3>融合题材</h3></div><small>建议 2 至 3 个</small></div>
          ...
          {step === 3 && <section className="opening-form-section" id="opening-protagonist-section" tabIndex={-1}>
            ...
            <label htmlFor="opening-world-background">时代背景<ImeTextarea id="opening-world-background" aria-label="时代背景" value={worldBackground} onChange={setWorldBackground} placeholder="例如：秦始皇统一六国后的咸阳，制度正在重建；尊重历史框架、允许合理架空的低武世界" rows={3} maxChars={800} /></label>
            {protagonists.map((protagonist, index) => <article className="protagonist-form-card" key={index}>
              ...
              <label htmlFor={index === 0 ? 'opening-protagonist-background' : `protagonist-background-${index}`}>角色背景<ImeTextarea id={index === 0 ? 'opening-protagonist-background' : `protagonist-background-${index}`} value={protagonist.background} onChange={(next) => updateProtagonist(index, { background: next })} placeholder="例如：现代青年穿越到秦朝，醒来后成为咸阳县衙一名没有根基的小吏" rows={2} maxChars={800} /></label>
              <label htmlFor={index === 0 ? 'opening-protagonist-family-background' : `protagonist-family-background-${index}`}>家庭背景<ImeTextarea id={index === 0 ? 'opening-protagonist-family-background' : `protagonist-family-background-${index}`} value={protagonist.familyBackground} onChange={(next) => updateProtagonist(index, { familyBackground: next })} placeholder="例如：出身边城小吏之家，父母早亡，与妹妹相依为命" rows={2} maxChars={800} /></label>
              <label htmlFor={index === 0 ? 'opening-protagonist-career-background' : `protagonist-career-background-${index}`}>职业背景<ImeTextarea id={index === 0 ? 'opening-protagonist-career-background' : `protagonist-career-background-${index}`} value={protagonist.careerBackground} onChange={(next) => updateProtagonist(index, { careerBackground: next })} placeholder="例如：县衙书吏，管户籍档案；修仙文可写宗门身份" rows={2} maxChars={800} /></label>
              <label htmlFor={index === 0 ? 'opening-protagonist-golden-finger' : `protagonist-golden-finger-${index}`}>金手指<ImeTextarea id={index === 0 ? 'opening-protagonist-golden-finger' : `protagonist-golden-finger-${index}`} value={protagonist.goldenFinger} onChange={(next) => updateProtagonist(index, { goldenFinger: next })} placeholder="主角独有的依仗或优势，没有可留空" rows={2} maxChars={800} /></label>
              <PersonalityPicker
        ...
        {step === 3 && <section className="opening-form-section story-direction-section">
          ...
          <label htmlFor="opening-start">开局（说清主角的处境、卷入的冲突、眼前的危机，最多800字）<ImeTextarea id="opening-start" aria-label="开局" maxChars={800} rows={3} value={openingStart} onChange={setOpeningStart} placeholder="例如：小职员重生回到被裁员当天，发现上司正拿着他的方案邀功，而三天后项目暴雷，他将替人背锅、被行业除名" /></label>
          <label htmlFor="story-ending">结局（选填；只写目前想到的阶段终点也可以，最多800字）<ImeTextarea id="story-ending" aria-label="结局" maxChars={800} rows={3} value={storyEnding} onChange={setStoryEnding} placeholder="例如：主角查清十年悬案为父翻案、扳倒幕后财团，从人人唾弃的罪人之子成为行业传奇，与并肩作战的爱人终成眷属" /></label>
          <label htmlFor="story-direction-custom">故事方向（选填，最多800字）<ImeTextarea id="story-direction-custom" aria-label="故事方向" maxChars={800} rows={3} value={storyDirection} onChange={setStoryDirection} placeholder="例如：主角从县衙小吏起步，以文书能力和现代常识解决真实危机，逐步进入帝国决策层" /></label>
        </section>}
      </div>
      <footer className="create-book-footer"><div><span>第{step}/3步 · {currentStep.title}</span>{missingByStep[step].length > 0 && <small className="create-book-requirements">{submitAttempted ? '请先补充' : '本步还需填写'}：{missingByStep[step].join('、')}</small>}</div><div><button className="secondary-button" type="button" onClick={handleCancel}>取消</button>{step > 1 && <button className="secondary-button" type="button" onClick={() => moveToStep((step - 1) as 1 | 2 | 3)}>上一步</button>}{step < 3 ? (step === 1 && !editing ? null : <button className="primary-button" type="button" onClick={() => moveToStep((step + 1) as 2 | 3)}>下一步</button>) : <button className="primary-button" type="button" disabled={busy || submitting} onClick={() => void submit()}>{busy || submitting ? (editing ? '正在保存' : '正在创建') : editing ? '保存修改' : '创建书籍'}</button>}</div></footer>
