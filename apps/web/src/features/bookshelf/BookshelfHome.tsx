...
import { bookDisplayTitle, bookStatusLabel, shortId } from '../../app/display-labels';
    ...
    <div className="bookshelf-scroll-region">
      {activeBooks.length === 0 ? <EmptyLibrary onCreate={onCreate} /> : <div className="book-cover-grid" aria-label="活动书籍">
        {activeBooks.map((book, index) => <article className="book-cover-card" key={book.bookId}>
          <button className="book-cover-open" type="button" aria-label={`打开《${bookDisplayTitle(book.title)}》`} onClick={() => onOpen(book.bookId)}>
            <span className={`book-cover-art cover-tone-${index % 5}`} aria-hidden="true"><BooksIcon /><b>{bookDisplayTitle(book.title).slice(0, 8)}</b><small>文秘写作</small></span>
            <span className="book-cover-copy"><strong>{bookDisplayTitle(book.title)}</strong><small>{bookStatusLabel(book.status)} · 正式内容版本 {book.canonRevision}</small><time dateTime={book.updatedAt}>最近更新 {formatShelfDate(book.updatedAt)}</time></span>
          </button>
          <button className="book-cover-menu" type="button" aria-label={`管理《${bookDisplayTitle(book.title)}》`} aria-expanded={bookMenuId === book.bookId} onClick={() => onToggleMenu(bookMenuId === book.bookId ? null : book.bookId)}><DotsThreeVerticalIcon /></button>
      ...
      {archivedBooks.length > 0 && <section className="home-archive">
        ...
        {archiveOpen && <div className="home-archive-list">{archivedBooks.map((book) => <article key={book.bookId}><span><strong>{bookDisplayTitle(book.title)}</strong><small>已归档 · {formatShelfDate(book.updatedAt)} · 编号 {shortId(book.bookId)}</small></span><div><button type="button" disabled={busy} aria-label={`恢复《${bookDisplayTitle(book.title)}》`} onClick={() => void onRestore(book)}><ArrowCounterClockwiseIcon />恢复</button><button className="danger-text-button" type="button" disabled={busy} aria-label={`彻底删除《${bookDisplayTitle(book.title)}》`} onClick={() => onPurge(book)}><TrashIcon />彻底删除</button></div></article>)}</div>}
      </section>}
    </div>
  </section>;
}
