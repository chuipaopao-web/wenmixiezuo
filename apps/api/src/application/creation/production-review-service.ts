...
import { parseProductionReview, type ProductionReview, type ReviewerRole } from '../../contracts/production-review.js';
...
export class ProductionReviewService {
  ...
  public merge(scope: BookScope, input: { panelId: string; manuscriptVersionId: string; revisionRound: number; reports: ProductionReview[] }): {
    ...
    const blocked = input.reports.some((report) => report.verdict === 'blocked')
      || issues.some((issue) => issue.severity === 'blocker')
      || riskLevels.some((level) => level === 'high' || level === 'blocked');
    const rewrite = !blocked && (input.reports.some((report) => report.verdict === 'rewrite')
      || issues.some((issue) => issue.severity === 'major')
      || riskLevels.some((level) => level === 'medium'));
    const hardActions = unique(issues.filter((issue) => issue.severity === 'blocker' || issue.severity === 'major')
      .map((issue) => `${issue.location}：${issue.requiredAction}（证据：${issue.evidence}；来源：${issue.reviewerRole}）`));
    const softActions = unique(issues.filter((issue) => issue.severity === 'minor' || issue.severity === 'observation')
    ...
    if ((blocked || rewrite) && input.revisionRound <= 2) this.repository.createRevisionOrder(scope, {
      ...
      disagreements: disagreements(input.reports), now: this.clock.now().toISOString()
    });
