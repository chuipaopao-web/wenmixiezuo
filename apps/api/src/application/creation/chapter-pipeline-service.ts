      ...
      const reviewPrompt = JSON.stringify({
          ...
          severityRubric: reviewer.role === 'fact'
            ? [
                ...
                '检索资料中的objective正史事实优先于人物说法、旧章节中的当时认知和conflicted/claim资料；人物被误认为死亡、失踪或失败后再次登场，若符合时间推进和已结算正史，属于剧情发展，不是正文自相矛盾。',
...
function outputTokenLimit(phaseKey: string): number {
  ...
  if (phaseKey.startsWith('editor-synthesis')) return 6_000;
  return 4_000;
}
...
export function factInferenceBoundaryReviewRules(): string[] {
  return [
    ...
    '不得臆造两个数量之间的一一对应关系。不同口径、不同时间范围或不同对象的数量可以同时成立，例如"累计获得过三次奖励"不等于"当前手里仍有三件奖励"。数量冲突必须是同一对象、同一指标、同一范围内出现两个不能同时成立的明确值，并同时引用两端正文证据；仅凭"可能对应不上"不得判major。',
