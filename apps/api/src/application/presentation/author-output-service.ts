...
import { sanitizeAuthorFacingText } from './author-facing-presentation.js';
...
export function prepareEffectiveOutput(raw: string): EffectiveOutputResult {
  ...
  if (structured !== null) {
    const visibleContent = sanitizeAuthorFacingText(renderStructuredReply(structured, false));
    const fullContent = sanitizeAuthorFacingText(renderStructuredReply(structured, true));
    return {
      visibleContent,
      fullContent,
  ...
  const visibleContent = rejectedMachinePayload
    ...
    : sanitizeAuthorFacingText(fallbackContent);
  return {
    visibleContent,
    fullContent: sanitizeAuthorFacingText(normalizedRaw),
