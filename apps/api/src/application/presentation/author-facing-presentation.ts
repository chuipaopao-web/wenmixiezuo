...
export function sanitizeAuthorFacingText(value: string): string {
  ...
  text = text.replace(/故事圣经\s*(?:中的?|里(?:的)?)?\s*premise/giu, workspaceFunctionLabel('basic') + '中的核心前提');
  text = text.replace(/\bstory_bible\b/giu, workspaceFunctionLabel('basic'));
  text = text.replace(/故事圣经/gu, workspaceFunctionLabel('basic'));
  text = text.replace(/圣经(?=中|里|版本|核心前提|premise)/giu, workspaceFunctionLabel('basic'));
  ...
  text = text.replace(/更新设定大纲中的核心前提为统一正史版本/gu, '按老板确认的版本更新设定中的核心前提');
  text = text.replace(
    /目前还没有正式确认的讨论结论，说明此前无正式确认决定落库。?/gu,
    '此前没有可直接沿用的正式决定。'
  );
  text = text.replace(/本书资料/gu, '书籍信息');
  text = text.replace(/设定大纲/gu, workspaceFunctionLabel('basic'));
  text = text.replace(/当前卷纲/gu, workspaceFunctionLabel('master'));
  text = text.replace(/事件设计/gu, workspaceFunctionLabel('event'));
  text = text.replace(/故事资料库/gu, workspaceFunctionLabel('library'));
  text = text.replace(/灵感讨论/gu, workspaceFunctionLabel('ideas'));
  return text.trim();
}