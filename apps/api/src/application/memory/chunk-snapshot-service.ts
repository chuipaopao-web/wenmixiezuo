...
export class ChunkSnapshotService {
  ...
  public build(scope: BookScope, source: ChunkSourceInput, canonRevision: number, failAt?: 'before_validate' | 'before_ready'): { snapshotId: string; chunkCount: number } {
    ...
    if (actualHash !== source.sourceHash) throw new Error('不可变来源哈希不匹配');
    const result = this.chunker.chunk(source.content);
    if (result.chunks.length === 0) throw new Error('来源没有可检索内容');
    const snapshotId = this.ids.next();
    const now = this.clock.now().toISOString();
    this.unitOfWork.run(() => {
      this.repository.createSnapshot(scope, {
        snapshotId, strategyVersion: result.policy.version, normalizationVersion: result.policy.normalizationVersion,
        embeddingTextPolicyVersion: result.policy.embeddingTextPolicyVersion, canonRevision, now
      });
      ...
      this.repository.addNode(scope, {
        ...
        title: source.title ?? null, byteStart: 0, byteEnd: result.sourceBytes, ordinal: 0, now
      });
      ...
      for (const chunk of result.chunks) {
        ...
        this.repository.addChunk(scope, {
          ...
          nextChunkId: chunk.nextOrdinal === null ? null : chunkIds[chunk.nextOrdinal]!, ordinal: chunk.ordinal,
      ...
      const coverage = { sourceBytes: result.sourceBytes, coveredBytes, excludedSeparators: result.excludedSeparatorRanges };
      const validation = { hashMatched: true, noOverlap: hasNoOverlap(result.chunks), parentContainsChildren: true, adjacencyValid: true };
      if (!validation.noOverlap) throw new Error('切片存在非预期重叠');
      this.repository.completeSnapshot(scope, snapshotId, {
        expected: 'building', next: 'validated', sourceCount: 1, nodeCount: parentIds.length + 1,
        chunkCount: result.chunks.length, coverageJson: JSON.stringify(coverage), validationJson: JSON.stringify(validation), now
      });
      this.repository.replaceFts(scope, snapshotId);
      if (failAt === 'before_ready') throw new Error('simulated-snapshot-probe-failure');
      this.repository.completeSnapshot(scope, snapshotId, {
        expected: 'validated', next: 'ready', sourceCount: 1, nodeCount: parentIds.length + 1,
        chunkCount: result.chunks.length, coverageJson: JSON.stringify(coverage), validationJson: JSON.stringify(validation), now
      });
    });
    return { snapshotId, chunkCount: result.chunks.length };
  }
}
