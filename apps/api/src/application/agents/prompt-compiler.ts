...
export class PromptCompiler {
  ...
  public compile(roleKey: CreativeRoleKey, task: { objective: string; mode: 'discussion' | 'formal_production' | 'review'; contextManifest: string[]; outputSchema: unknown }): CompiledRolePrompt {
    ...
    const publicContract = {
      title: contract.shortTitle,
      member: contract.memberName,
      summary: contract.publicSummary,
      professionalIdentity: contract.professionalIdentity,
      craftStrengths: contract.craftStrengths,
      workingMethod: contract.workingMethod,
    ...
    const system = [`你是${contract.memberName}（${contract.shortTitle}）。`,
      `专业身份：${contract.professionalIdentity}`,
