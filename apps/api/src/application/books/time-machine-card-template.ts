...
export const TIME_MACHINE_CARD_TEMPLATE_REVISION='tm2-card-2';
...
export function planningMaterial(fields:unknown,intent:string,methods?:unknown):string {
 return JSON.stringify({bookMaterial:{status:'已确认来源的摘要，遗漏不代表不存在',fields},storylineIntent:{status:'作者选择与补充，属于未来设计要求，不是正文事实',text:intent},...(methods===undefined?{}:{methodReference:{status:'成员选用的方法和补查资料；方法可选，不作为本书事实',content:methods}})});
}
