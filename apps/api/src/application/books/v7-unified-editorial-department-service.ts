...
type Presence = 'ready' | 'working' | 'leave' | 'candidate' | 'unbound';
...
export class V7UnifiedEditorialDepartmentService {
  ...
  public get(ownerId: string): V7UnifiedEditorialDepartmentView {
    ...
    for (const slot of MEMBER_SLOTS.filter(member => !member.legacy&&!members.some(m=>m.memberKey===member.memberKey))) {
      ...
      members.push({ memberKey: slot.memberKey, displayName: slot.displayName, role: role.publicName,
        responsibility: role.publicResponsibility, capabilities: [...V7_ROLE_CONTRACTS.find(contract => contract.roleKey === slot.roleKey)!.capabilities], presence: 'unbound',
        statusText: '预留成员，尚未绑定模型。', currentWork: null, completedCount: 0 });
    }
    return {
      summary: {
  ...
  private memberView(ownerId: string, member: V7EffectiveMember, activeSince: string): V7UnifiedEditorialMemberView {
    ...
    const presence: Presence = currentWork !== null ? 'working' : available ? 'ready' : MEMBER_SLOTS.some(s=>s.memberKey===member.memberKey&&!s.legacy)?'candidate':'leave';
    return {
      ...
      statusText: presence === 'working' ? `我正在处理${currentWork}，完成后马上交稿。`
        : presence === 'candidate' ? '模型已接入，等待具体岗位验证后自动接单。'
