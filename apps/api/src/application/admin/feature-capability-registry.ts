...
const C: FeatureCapabilityDefinition[] = [
  ...currentModule('identity-account','账号与身份','author','登录页 / 个人中心',
    ['apps/web/src/features/auth/AuthScreen.tsx','apps/web/src/features/account/PersonalCenterDialog.tsx','apps/api/src/http/account-routes.ts'],[
  ...
  ...currentModule('bookshelf-lifecycle','书架与书籍生命周期','author','我的书籍',
    ['apps/web/src/app/App.tsx','apps/web/src/features/bookshelf/BookLifecycleDialogs.tsx','apps/api/src/application/books/book-lifecycle-service.ts'],[
  ...
  ...currentModule('opening-profile','开书资料与接续','author','建书弹窗 / 顶部书籍信息',
    ['apps/web/src/features/onboarding/CompleteCreateBookDialog.tsx','apps/web/src/features/core-workflow/CoreWorkflowWorkspace.tsx','apps/api/src/application/books/book-profile-view-service.ts'],[
  ...
  ...currentModule('setting-workspace','非剧情设定','author','设定',
    ['apps/web/src/features/planning/SettingCatalog.tsx','apps/web/src/features/planning/SettingCollaborationPanel.tsx','apps/api/src/application/knowledge/setting-outline-workspace-service.ts'],[
  ...
  ...currentModule('storyline-growth','生长式故事线','author','故事线',
    ['apps/web/src/features/core-workflow/StorylineWorkspace.tsx','apps/web/src/features/core-workflow/StorylineBoard.tsx','apps/api/src/application/planning/story-thread-service.ts'],[
  ...
  ...currentModule('volume-planning','分卷方向','author','分卷',
    ['apps/web/src/features/core-workflow/VolumeExpressionWorkspace.tsx','apps/web/src/features/planning/VolumePlanningPanel.tsx','apps/api/src/application/planning/volume-plan-service.ts'],[
  ...
  ...currentModule('event-chain','当前卷事件链','author','事件',
    ['apps/web/src/features/core-workflow/EventRoleWorkspace.tsx','apps/web/src/features/planning/EventPlanningPanel.tsx','apps/api/src/application/planning/story-event-service.ts'],[
  ...
  ...currentModule('chapter-outline','章链与详细章纲','author','章节',
    ['apps/web/src/features/planning/EventChapterPlanningPanel.tsx','apps/web/src/features/manuscript/ManuscriptWorkspace.tsx','apps/api/src/application/planning/event-chapter-outline-service.ts'],[
  ...
  ...currentModule('manuscript-production','单章正文与审校','author','章节 → 正文',
    ['apps/web/src/features/manuscript/ManuscriptWorkspace.tsx','apps/api/src/application/creation/chapter-pipeline-service.ts','apps/api/src/application/creation/chapter-approval-service.ts'],[
      ['chapter-draft-generation','单章正文生成','按确认章纲生成正文。'],['writer-selection','主笔选择','选择或追加主笔独立生成。'],
      ['manuscript-versioning','正文完整版本','修改产生完整新版本。'],['multi-review','三类独立审查','事实、文学和体验分别审查。'],
      ['editor-synthesis','主编审查汇总','主编生成可执行汇总。','current'],['chapter-approval','章节确认定稿','确认正文版本后结算。']]),
  ...currentModule('settlement-continuity','结算与后续衔接','author','章节 / 事件 / 分卷结算',
    ['apps/web/src/features/planning/SettlementFollowUpCard.tsx','apps/api/src/application/planning/creation-settlement-service.ts','apps/api/src/application/planning/settlement-follow-up-service.ts'],[
  ...
  ...currentModule('knowledge-library','资料库与故事知识','author','资料库',
    ['apps/web/src/features/library/LibraryWorkspace.tsx','apps/web/src/features/library/StoryKnowledgeWorkspace.tsx','apps/web/src/features/graph/ProjectionWorkspace.tsx'],[
  ...
  ...currentModule('naming-tools','命名工具','author','命名',
    ['apps/web/src/features/naming/NamingWorkspace.tsx','apps/api/src/http/domain-routes.ts'],[
      ['name-generation','名称生成','为人物、地点和组织生成候选。'],['name-library','命名记录','保存和复用已采用名称。']]),
  ...currentModule('editorial-team','AI 编辑部','author','团队',
    ['apps/web/src/features/core-workflow/EditorialTeamWorkspace.tsx','apps/api/src/application/agents/team-template-service.ts','apps/api/src/application/agents/ai-node-batch-service.ts'],[
      ['team-roster-25','7 类岗位、初始 25 名成员','后台可继续扩充。'],['team-role-categories','岗位分组','主编、副编、编剧、主笔和三类审查席。'],
      ['team-member-choice','成员选择与追加','默认一人，可更换或追加。'],['team-parallel-fairness','同资料包公平比较','同批成员资料一致且输出隔离。'],
      ['team-status-cost','状态与消耗等级','显示供应公司、消耗等级和真实状态。']], 'previous-production'),
  ...currentModule('task-center','任务中心','author','任务',
    ['apps/web/src/features/tasks/TaskWorkspace.tsx','apps/api/src/application/tasks/task-service.ts','apps/web/src/features/shared/task-presentation.ts'],[
  ...
  ...currentModule('ideation','灵感','author','灵感',
    ['apps/web/src/features/ideation/IdeationWorkspace.tsx','apps/api/src/application/ideation/ideation-service.ts'],[
      ['idea-capture','灵感记录','记录未进入正式规划的想法。'],['idea-to-book-context','灵感用于创作','选择后作为本书资料来源。']]),
  ...currentModule('settings-portability','设置与可携带性','author','设置',
    ['apps/web/src/features/settings/SettingsDialog.tsx','apps/api/src/application/portability/book-portability-service.ts'],[
      ['book-export','书籍导出','导出本人书籍数据。'],['book-import','书籍导入','校验并导入数据包。'],['writing-preferences','创作偏好','维护作者可见偏好。']]),
  ...currentModule('feedback','意见与问题反馈','author','意见反馈',
    ['apps/web/src/features/feedback/FeedbackDialog.tsx','apps/api/src/http/domain-routes.ts'],[
  ...
  ...currentModule('author-membership','作者会员与用量','author','个人中心 / 会员门禁',
    ['apps/web/src/features/shared/membership-gate.tsx','apps/web/src/features/account/PersonalCenterDialog.tsx','apps/api/src/infrastructure/security/membership-service.ts'],[
  ...
  ...currentModule('admin-dashboard','后台运营总览','admin','独立后台 → 总览',
    ['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/http/admin-console-routes.ts'],[
  ...
  ...currentModule('admin-compute','后台算力','admin','独立后台 → 算力',
    ['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/http/admin-platform-routes.ts'],[
  ...
  ...currentModule('admin-ai-governance','后台模型、成员与 Skill','admin','独立后台 → 模型 / 创作模板',
    ['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/http/admin-console-routes.ts','apps/api/src/application/agents/agent-skills-v6.ts'],[
      ['admin-model-scheme','平台模型方案','管理员绑定供应商和模型。'],['admin-member-binding','成员独立绑定','管理成员模型、供应商和状态。'],
      ['admin-member-expand-26','扩充第 26 名及更多成员','25 名不是上限。','current'],['admin-skill-registry','Skill 版本台账','查看三层 Skill 版本与哈希。','current'],
  ...
  ...currentModule('admin-templates','后台创作模板','admin','独立后台 → 创作模板',
    ['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/application/agents/creative-templates-v6.ts','apps/api/src/application/planning/hidden-narrative-methods.ts'],[
  ...
  ...currentModule('admin-prompts','后台提示词','admin','独立后台 → 提示词',
    ['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/application/agents/admin-ai-trigger-catalog.ts','apps/api/src/application/agents/prompt-compiler.ts'],[
  ...
  ...currentModule('feature-governance','功能资产守恒','admin','独立后台 → 功能台账',
    ['apps/api/src/application/admin/feature-capability-registry.ts','apps/web/src/features/admin-console/FeatureCapabilitiesPage.tsx','scripts/quality/verify-feature-capability-registry.ts'],[
...
const H: FeatureCapabilityDefinition[] = [
  { id:'legacy-information-page',moduleId:'opening-profile',moduleName:'开书资料与接续',surface:'author',name:'独立信息页',description:'旧版独立一级信息页。',currentAvailable:false,currentEntry:null,
    evidence:['apps/web/src/features/core-workflow/CoreWorkflowWorkspace.tsx'],introduced:'stable-baseline',statusOverrides:{'stable-baseline':'relocated'},previousEntry:'信息页',
  ...
  { id:'legacy-planning-workspace-shell',moduleId:'planning-legacy',moduleName:'旧规划工作台',surface:'author',name:'旧规划总壳层',description:'旧版单一规划工作台。',currentAvailable:false,currentEntry:null,
    evidence:['apps/web/src/features/core-workflow/CoreWorkflowWorkspace.tsx'],introduced:'stable-baseline',statusOverrides:{'stable-baseline':'replaced'},previousEntry:'PlanningWorkspace',
  ...
  { id:'legacy-team-workspace',moduleId:'editorial-team',moduleName:'AI 编辑部',surface:'author',name:'旧团队工作台',description:'旧版团队页面。',currentAvailable:false,currentEntry:null,
    evidence:['apps/web/src/features/core-workflow/EditorialTeamWorkspace.tsx'],introduced:'stable-baseline',statusOverrides:{'stable-baseline':'replaced'},previousEntry:'TeamWorkspace',
    replacement:'EditorialTeamWorkspace',decision:'由可扩充的 7 类岗位、初始 25 名成员页替代。' },
  { id:'legacy-fixed-15-member-roster',moduleId:'editorial-team',moduleName:'AI 编辑部',surface:'author',name:'固定 15 人成员表',description:'旧版写死 15 名成员。',currentAvailable:false,currentEntry:null,
    ...
    replacement:'7 类岗位、初始 25 名、后台可扩充',decision:'旧人数规则与最新决定冲突，已被替代。' },
  { id:'author-protected-role-prompt-viewer',moduleId:'admin-prompts',moduleName:'后台提示词',surface:'admin',name:'作者端岗位提示词查看',description:'旧团队页向作者显示完整内部岗位提示词。',currentAvailable:false,currentEntry:null,
    evidence:['apps/web/src/features/admin-console/AdminPages.tsx','apps/api/src/http/admin-console-routes.ts'],introduced:'stable-baseline',statusOverrides:{'stable-baseline':'relocated'},previousEntry:'团队 → 查看完整提示词',
  ...
  { id:'author-agent-prompt-preference-editor',moduleId:'editorial-team',moduleName:'AI 编辑部',surface:'author',name:'作者逐成员提示词偏好',description:'旧团队页允许直接改写成员内部提示。',currentAvailable:false,currentEntry:null,
    evidence:['apps/api/src/application/agents/agent-prompt-preference-service.ts','apps/web/src/features/core-workflow/EditorialTeamWorkspace.tsx'],introduced:'stable-baseline',statusOverrides:{'stable-baseline':'retired'},previousEntry:'团队 → 成员提示词偏好',
...
export const FEATURE_BASELINE_SNAPSHOTS: Record<FeatureBaselineKey, readonly string[]> = {
  'previous-production': C.filter((item) => item.introduced !== 'current').map((item) => item.id),
