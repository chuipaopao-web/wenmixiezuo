...
import {
  ...
  type ChatAttachmentMediaKind,
  type ChatAttachmentParseStatus,
...
interface ParseResult {
  ...
  status: Exclude<ChatAttachmentParseStatus, 'discarded'>;
  error: string | null;
}

interface MediaSpec {
  kind: ChatAttachmentMediaKind;
  extension: string;
  mimeType: string;
}
...
export class ChatAttachmentService {
  ...
  public async upload(scope: BookScope, input: { filename: string; mimeType: string; buffer: Buffer }): Promise<ChatAttachmentRecord> {
    ...
    if (input.buffer.length > MAX_CHAT_ATTACHMENT_BYTES) throw new DomainError(errorCodes.validation, '单个附件不能超过20 MiB');
    ...
    const repository = new ChatAttachmentRepository(this.database, scope);
  ...
  public get(scope: BookScope, attachmentId: string): ChatAttachmentRecord {
    return new ChatAttachmentRepository(this.database, scope).require(attachmentId);
  }

  public readSource(scope: BookScope, attachmentId: string): { record: ChatAttachmentRecord; buffer: Buffer } {
  ...
  public discard(scope: BookScope, attachmentId: string): ChatAttachmentRecord {
    return new ChatAttachmentRepository(this.database, scope).discard(attachmentId);
  }
}

async function parseBuffer(kind: ChatAttachmentMediaKind, buffer: Buffer): Promise<ParseResult> {
