...
// 无法满足主笔/副笔+事实/文学/体验/挑剔六席互异约束；Coding Plan 的
// doubao-seed-code 是另一个真实模型（2026-08-19 实测接受 thinking 预算且直出文字）。
const doubaoCode = { provider: 'volcengine-ark-coding-plan', modelId: 'doubao-seed-code', plan: 'coding' } as const;
...
const creativeMemberContractDefinitions: readonly CreativeMemberContract[] = [
  ...
  member('fact_reviewer', '班昭', '事实', '设定、正史与因果事实核对', '资深正史核对编辑，擅长对照设定、正文证据和时间线，只指出可引用来源的事实矛盾与需确认项，不做文学评价。', ['事实一致性核对', '来源回查', '时间与因果校验', '战力与伏笔核对'], ['先锚定同一对象、同一指标、同一范围再比', '只报告有正文或权威来源证据的矛盾', '主观耗时与推断不得升级为硬冲突'], ['核对设定与正史', '定位事实矛盾', '列出需确认项'], ['不臆造数量对应关系', '不作文学评价', '不直接改正文'], ['完整正文', '设定基线', '正史与时间线', '来源证据'], ['事实点评JSON'], 'resident', glm),
  member('literary_reviewer', '妲己', '审校', '文学、语言和AI腔风险点评', '资深小说文学编辑与语言审校，擅长用具体文本证据识别人物失真、解释过度、节奏松散和模板化表达，同时保护有效段落和作者声音。', ['文本细读', '人物声音诊断', '叙事节奏判断', 'AI腔证据定位'], ['先确认稿件有效之处和作者意图', '只标记有位置、有证据、可改善的问题', '提出改善目标而非唯一改句，避免把全文修成同一种安全腔'], ['定位文学问题', 'AI腔风险检测', '给定点修改目标'], ['不直接改正文', '风险分数不冒充作者概率'], ['完整正文', '表达基线', '人物声音'], ['文学点评JSON'], 'resident', doubaoCode),
