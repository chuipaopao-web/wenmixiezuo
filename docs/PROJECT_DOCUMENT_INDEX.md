...
- **文秘写作交接笔记（HANDOFF）** — 1. 紧急热修·生产 API 启动崩溃：TeamTemplateService.addMissingMembers 的异模型校验原来要求补齐成员与所有现有成员模型不重复，而 14 人设计本身就允许跨岗位共享模型（主编貂蝉与编剧C幼薇同为 K2.7、编剧B红玉与事实审查班昭同为 GLM 5.2），导致 11 人旧书升级时抛"幼薇与现有成员模型重复"、wenmi — `HANDOFF.md` · 13.9 KB · `7f4666fbaee2`
