...
- **分层创作系统实施方案与验收清单** — 本文件把已经确认的产品决定集中转换成可实施、可测试、可验收的开发合同。它不能替代项目章程和生效决定；发生冲突时，按 AGENTS.md 规定的权威顺序处理。 — `docs/LAYERED_CREATION_IMPLEMENTATION_AND_ACCEPTANCE.md` · 57.4 KB · `778b06d6f8bc`
...
- **文秘写作交接笔记（HANDOFF）** — 1. 卷规划前置门禁恢复（DEC-CURRENT-092）：普通作者尚未完成设定定稿时，卷页不再显示一个注定失败的“开始规划第一卷”；改为读取最新设定质检状态，显示需要作者决定的硬问题数量，并提供“回到设定处理”直接切页。后端设定基线门禁保持不变，不替作者自动确认矛盾。手机端卡片单列、按钮满宽。卷规划定向4项、Web与测试TypeScript、全量184文件 — `HANDOFF.md` · 51.7 KB · `c528db585cff`
