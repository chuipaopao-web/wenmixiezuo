...
本目录只收录当前版本实际生效的产品、工作流、架构、质量和 Codex 规则，共 **38** 份。文档卡片与全文均由源文件实时生成。
...
- **当前API与事件契约** — 确认开书会原子创建书籍、定位版本、团队、模型绑定、预算、设定工作区和首个设定任务。 — `docs/API.md` · 12.1 KB · `a3ff4fd1cff9`
- **系统架构** — 统一账号的模块化单体加独立 Worker： — `docs/ARCHITECTURE.md` · 8.7 KB · `83ed7306db62`
...
- **创作工作流 V2 完整设计** — 活动事件链 → 当前事件大纲版本 — `docs/CREATION_WORKFLOW_V2_DESIGN.md` · 16.7 KB · `499b0773b395`
- **数据模型规格** — 规划对象描述未来，不直接写入正史。活动版本必须引用活动上游版本；上游改变后旧候选标记过期。 — `docs/DATA_MODEL.md` · 11.1 KB · `8491080cb7f7`
- **当前生效决定** — 本文只记录正在驱动当前版本的决定。 — `docs/DECISIONS.md` · 139.8 KB · `3428c20962da`
...
- **当前开发与验收计划** — 本路线只记录当前版本。完成以代码、迁移、测试、构建和本地运行证据为准；真实模型文学质量另按E3/E4积累。 — `docs/DEVELOPMENT_ROADMAP.md` · 5.5 KB · `dbb295cca1d2`
...
- **分层创作系统实施方案与验收清单** — 本文件把已经确认的产品决定集中转换成可实施、可测试、可验收的开发合同。它不能替代项目章程和生效决定；发生冲突时，按 AGENTS.md 规定的权威顺序处理。 — `docs/LAYERED_CREATION_IMPLEMENTATION_AND_ACCEPTANCE.md` · 57.5 KB · `ccf230df7ff5`
...
- **文秘写作交接笔记（HANDOFF）** — 1. 卷规划前置门禁恢复（DEC-CURRENT-092）：普通作者尚未完成设定定稿时，卷页不再显示一个注定失败的“开始规划第一卷”；改为读取最新设定质检状态，显示需要作者决定的硬问题数量，并提供“回到设定处理”直接切页。后端设定基线门禁保持不变，不替作者自动确认矛盾。手机端卡片单列、按钮满宽。卷规划定向4项、Web与测试TypeScript、全量184文件 — `HANDOFF.md` · 52.1 KB · `764aeda0f528`
- **文秘写作项目总入口** — 这是老板和 Codex 查看当前项目规则的固定入口，只指向当前生效文档和当前Skill。 — `PROJECT_HANDBOOK.md` · 6.6 KB · `1ab0c4e3d2a7`
- **文秘写作** — 文秘写作是一套本地优先的长篇小说 AI 协作平台。作者通过明确的创作对象推进工作，AI 成员直接给出独立方案、融合候选、正文和审查报告，不使用聊天推动创作。 — `README.md` · 2.9 KB · `661024d16280`

## Codex 工作规则

共 3 份。桌面阅读中心可点击卡片直接阅读全文。

- **文秘写作开发协作规则** — 1. 老板最新明确决定； — `AGENTS.md` · 8.3 KB · `f2ac59c24e0e`
- **文秘写作当前知识地图** — 文秘写作是使用统一账号体系、支持本机测试和后续公网部署的长篇小说AI协作平台。作者始终拥有最终决定权，AI通过明确对象提供独立方案、融合候选、正文和审查报告，不使用聊天推动流程。 — `KNOWLEDGE.md` · 3.2 KB · `a9b3ca41b040`
...
- **卷—事件工作流的上下文与混合检索** — 上下文由确定性的 ContextCompiler 按任务编译，不由某个拟人岗位垄断数据库，也不依赖聊天记忆。 — `.agents/skills/wenmi-longform-quality/references/context-and-rag.md` · 9.4 KB · `4d999c572a83`
...
- **文秘写作长篇质量审查** — name: wenmi-longform-quality — `.agents/skills/wenmi-longform-quality/SKILL.md` · 20.3 KB · `70d2e88d2851`
