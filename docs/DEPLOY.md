...
- **回滚迁移目录**：增量迁移也要验证旧代码能读取已迁移数据库。当前迁移器拒绝目录中缺失的已应用迁移；切换前准备独立兼容回滚包，在已迁移副本上验证迁移与隔离启动，不能仅保留旧源码就认定能回滚。不要逆向删除生产迁移记录。
- **心跳与静态验证**：新Worker按实际PID、版本和新鲜度限时等待，不能在首次心跳之前立即判失败，旧心跳的ready也不代表新进程可用。静态原始产物按清单严格验证；为已打开页面保留的旧哈希资源另与旧版本逐项比较，不将其混入新清单或误判为新产物损坏。
...
1. 本机按 [ACCEPTANCE.md](ACCEPTANCE.md) 的统一风险矩阵完成受影响验证；全量测试由具体高风险事实或老板明确要求触发，独立加法表/索引不自动全量。使用 `git -c core.autocrlf=false -c core.eol=lf archive` 生成发布包。
2. 在唯一暂存目录完成受影响服务构建和迁移预检；迁移只向后兼容，已合并迁移字节变化立即停止。构建后以实际服务账号`wenmi`验证模块读取和受影响流程，不用root探针替代；root配合`umask 077`生成的新文件须在未上线目录修正归属后再检验。第130批曾因此启动失败并回退。
3. 正式迁移前备份数据库。查询 `tasks`，`working`、`queued`、`pending`、`waiting_confirmation` 连续30秒为0，并在每次重启前立即复核；只等待，不取消/暂停/改写作者任务。
4. 逐个原子切换并重启受影响服务，每一步检查active、日志、健康和任务恢复；失败立即回滚，不继续扩大。若连续启动失败触发systemd频率保护，确认已回滚的代码路径和安全窗口后清除对应服务失败计数，按API健康→Worker新心跳顺序完成代码回滚；这一步不恢复数据库或改写作者任务。
...
- 老板已于2026-09-05明确授权本次及后续常规安全部署，包括向下述生产目标上传项目私有源码包、构建产物和部署脚本。满足根AGENTS第7节范围及门禁后直接执行，不重复询问上传/上线；不把此授权用于其他服务器、作者作品外传或生产数据恢复。工具安全审查不能由项目文档关闭，实际拒绝须如实说明并按原因处理。
