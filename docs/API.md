...
DEC-066补充本书资料、规划状态、表达调色板和设定确认读写契约。表达调色板可为空，但系统仍保存可追溯的表达策略版本；总纲、卷纲、章纲使用独立确认入口，任一入口不得顺带生成其他层级；越级调用返回缺项列表而不是内部错误。DEC-069进一步规定新确认的剧情总纲使用 `stage_master_v2`：两名编剧各自提交完整阶段结构后才允许主编综合；阶段章节范围必须从第1章开始、连续、无重叠且无空洞。
...
历史总纲继续只读兼容；任何新确认、作者编辑后再确认或编剧重做都必须生成 `stage_master_v2`，不得用全书简介、卷纲或逐章事件列表代替。
...
- 主线优先来自已选卷纲/总纲的阶段结构及真实故事弧结算；情绪只来自明确情绪字段或可验证的结构化情绪描述；支线、钩子、伏笔和信息差只从相应明确来源投影。
