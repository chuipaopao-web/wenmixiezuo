...
- `GET /api/v1/opening-taxonomy` 同时返回 `boundaryGroups`：感情与关系、主角体验、内容尺度、结构与结局四组共24项“必须遵守”快捷边界。客户端可以与自定义边界合并提交，但最终仍由 `openingBlueprint.mustFollow` 的1—12条服务端校验兜底。
...
`POST /api/v1/books/{bookId}/attribute-formulas` 接受可选 `category` 字段；省略时按旧数据兼容为 `uncategorized`。返回记录始终包含 `category`。设定原文通过现有消息接口提交并点名设定成员，模型结果保持候选权威，不由该接口直接写入正史。
