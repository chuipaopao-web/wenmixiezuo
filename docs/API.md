...
建书确认必须包含开书草稿版本，防止确认旧版本。当前开书资料校验频道、一个主分类、1—8位完整初始主角、20—800字故事方向、最多8个题材、至少2个主要标签及作品边界；主要标签不设选择数量上限，自定义标签不能冒充分类键。旧客户端提交的目标读者字段继续兼容，但新书不再要求填写。故事方向和软标签只属于规划参考，不得声称已形成正史生成门禁。叙事视角仍在首个正式写作工单前确认；“情绪引擎”不属于开书API。
...
| GET | `/books/{bookId}/artifacts` | 按类型查询创作方案、设定大纲和剧情大纲 |
...
```json
{
  ...
  "majorStages": [{
    ...
    "stageSummary": "阶段结束时人物、局势与资源的变化",
    "pendingThreads": ["尚未回收的信息或伏笔"],
    "followUpDirection": "下一阶段承接方向"
  }],
  "endingDirection": "结局方向",
  "storyPromises": ["需要持续兑现的作品承诺"],
  "openQuestions": ["仍需老板决定的问题"]
}
```

历史总纲继续只读兼容；任何新确认、作者编辑后再确认或编剧重做都必须生成 `stage_master_v2`，不得用全书简介、历史卷纲或逐章事件列表代替。`volume_outline` 为历史审计类型，公共作者接口不得新建、确认或列出该类型。
...
- `POST /api/v1/books/drafts`：除兼容旧客户端字段外，当前Web入口提交 `openingBlueprint`。服务端校验男/女频道、一个主分类、1—8位完整初始主角、20—800字 `storyDirection`、0—8个题材、必须遵守以及至少2个主要标签；主要标签不设选择数量上限，自定义标签最多13个。`storyDirection` 应概括开篇处境、启动事件、主角目标和主要阻力，但只是可修订规划参考。旧客户端的0—3个同频道 `auxiliaryCategoryKeys` 会兼容折算成题材，新Web不再提交该字段。未知自定义标签允许，但不能冒充目录分类键。`targetAudience` 仅为旧书和旧客户端兼容字段，可为空。
...
`POST /api/v1/books/{bookId}/attribute-formulas` 接受可选 `category` 字段；省略时按旧数据兼容为 `uncategorized`。返回记录始终包含 `category`。设定原文通过现有消息接口提交并点名设定成员；真实回复会创建设定大纲候选版本并显示在基本设定中，未经作者确认不会替换活动版本或进入正史。

## 22. 叙事图谱作者投影

- `GET /api/v1/books/:bookId/graph` 返回人物关系及五类作者叙事投影。作者视图只返回或只展示 DEC-068 白名单字段，不暴露内部ID、原始JSON、模型审校载荷或来源指纹。
- `POST /api/v1/books/:bookId/projections/rebuild` 只重建可派生投影，不改写规划、正史和正文。响应中的 `count` 是真实生成数量，不按“章节数 × 五类 × 两轨”虚报。
- 主线优先来自已选剧情总纲的阶段结构及真实故事弧结算；情绪只来自明确情绪字段或可验证的结构化情绪描述；支线、钩子、伏笔和信息差只从相应明确来源投影。历史卷纲不得继续成为新投影的优先来源。
...
- `GET /api/v1/books/:bookId/setting-outline-workspace/:itemKey/collaboration`：读取当前项最近一次三席提案任务、讨论状态、三份独立方案、冻结的真实成员/模型来源、最近主编修订任务、历史轮次和影响说明。该接口只读，不创建任务、不调用模型、不修改候选或正史；重复读取和刷新必须幂等。
...
- 所有记录按书籍作用域校验。工作区写接口只更新候选工作状态；单项确认不改写正文或正史，整份设定基线确认仍是唯一正式版本入口。
- Web在当前项内通过既有消息接口启动活动主编加两名异模型编剧的独立提案任务，不跳转页面。作者已有设定原文先写入 `author_planning_inputs`，再以软参考随本轮消息提交；作者选案后只调度主编收束。失败重试复用原任务检查点，已经成功的席位不得重复调用。
...
- `POST /api/v1/books/:bookId/continuation-imports/preview`：请求体为 `{ sourceName, text }`；文本为1—5,000,000个规范化字符。只建立导入预览和不可变暂存源，不创建章节、正文、正史、事实、索引或模型调用。返回导入ID、源哈希、解析器版本、逐章标题/字符数/警告和纳入开关。
