...
- [ ] 记录DEC-038与 `DR-20260720-01`，明确“一个可见身份、多个内部来源”。
- [ ] 完成正确性/安全与创造性/输出质量两轮审查。
- [ ] 锁定历史兼容、零模型调用、直接成员回复和错误脱敏合同。
...
- [ ] 证明问候由小文秘书本地回复而非创建主编模型任务。
- [ ] 证明任务/资料命令执行真实本地动作并返回自然话术。
- [ ] 证明历史 `system` 消息显示“小文秘书”且不显示系统头像或系统名称。
- [ ] 证明未处理服务错误使用友好、无敏感细节的说明。
...
- [ ] 将可见通知写为 `local_assistant_notice`，内部 `system` 来源只保留兼容性。
- [ ] 增加本地问候、身份说明、真实任务概览与资料库导航。
- [ ] 为所有路由结果提供自然、简短、有证据的话术，补齐点名成员等旧回退分支。
- [ ] 前端统一头像、名称、错误提示和右栏身份，并按返回动作打开任务或资料库。
...
- [ ] 运行目标红绿测试、类型检查、全量测试和三端构建。
- [ ] 验证空库/升级迁移幂等、Repository隔离和历史消息兼容。
- [ ] 启动文秘写作自身服务，验证Web/API/Worker与恢复路径，不触碰智囊团。
- [ ] 保存证据、提交、运行正式验收并推送私有远程仓库。
