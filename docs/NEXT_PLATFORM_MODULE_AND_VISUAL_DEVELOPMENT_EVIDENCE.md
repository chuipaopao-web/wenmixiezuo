...
- 第 0–15 项已全部实现、核查、测试并勾选；清单未勾选项为 0。生产发布、生产域名视觉复核、核心冒烟和观察窗口均已通过。
...
- 发布对象：wm-rolling-storyline-r2-20260823-114755-f324dfeb；Git 提交 f324dfebe7203bd34ff5893d0a4848daf26e1add；迁移 0068_rolling_storyline_growth.sql；前一版本及运行时回滚基线 wm-v6-core-r1-20260823-035846-d98dc814；负责人为本次 Codex 开发任务。
- 发布包：wm-rolling-storyline-r2-20260823-114755-f324dfeb.tar，SHA-256 0222859ae0788e12f4690c2d00c6790751f927a7a9cdb71ab88041e8d19d50fc；生产同机暂存构建与本地构建的六个最终 Web 哈希资源一致。
- 迁移预检：对生产数据库一致性副本首次应用 0068、第二次应用 0 条；迁移序号 68、产品 schema 66、七张新增表、PRAGMA integrity_check=ok。正式迁移前保存 SQLite 一致性备份、运行时代码、Web 入口及 SHA-256；manuscript_versions、stage_settlements、model_calls、usage_ledger 迁移前后哈希一致。
- 在途任务：2026-08-23T03:58:08Z 至 03:58:39Z 连续七次采样均为 0，停止 Worker 后及停止 API 后立即复核仍为 0；没有取消、暂停或改写作者任务。历史上一个已失败任务遗留的孤立 working 调用不属于活动任务，未被修改。
- 正式迁移后的首轮发布脚本因把七张新增表写成错误期望名称而保护性停止；数据库迁移本身成功且完整，旧运行时未切入新前端。经老板明确授权后采用前向完成发布，新代码与新迁移目录一起原子切换；没有恢复、删除或改写作者数据。
- API、Worker 逐个启动并通过本机及公网健康检查；最终 API/Worker/Caddy 均为 active，/health 返回 Worker ready、canStartModelTasks=true 和新发布号。正式发布后队列四项为 0|0|0|0，新增失败任务 0、故事线候选幂等重复 0、数据库完整性 ok。
- Web 在最终发布暂存目录构建；旧哈希资源保留，仅原子切换 index.html。六个最终 JS/CSS 资源在主站均为 200，主站与独立后台为 200；真实 /api/v1/books、后台总览/用户/AI 治理及本批核心路由未登录均返回 401，而不是 404。
- 最终发布目录隔离复跑 9 个核心文件、44 个用例全部通过，覆盖零故事线、阶段终点、开放问题、卷结算投影、主编候选、25 人团队、同批公平、失败席重试、ContextPack、付费/收入、用户书籍/失败位置、Skill/模板治理。
- 生产域名视觉：主站与后台登录壳 16 张截图无资源失败；生产域名真实加载线上静态资源、业务请求拦截到本机隔离 QA 库后完成 10 页 × 8 视口共 80 张认证截图，生产数据零写入。流程栏 0、横向溢出 0、浏览器异常 0、成员数 25、手机两排导航通过；50 个桌面稳定区域最低相似度 96.66%、平均 97.25%。
- 观察窗口：发布后 7 分钟再次检查健康、队列、数据库、幂等和日志；API/Worker journal 错误 0、敏感日志命中 0、按发布时间过滤的主站/后台 5xx 均为 0。证据位于 artifacts/next-platform-20260823/production-release/、production-visual/ 和 production-authenticated-visual/。
