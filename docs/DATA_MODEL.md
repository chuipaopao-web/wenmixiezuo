...
- `v7_setting_batches`：V7 设定编辑部的持久任务壳。普通设定批次至少有一条 `v7_setting_item_jobs`；主编设定清单复用同一租约与恢复设施，但没有条目工单，并在 `custom_items_json.taskKind=catalog_recommendation` 中保存公开阶段，在 `selected_items_json` 中保存三类目录结果。清单任务冻结调用时的开书版本、开书哈希、目录请求哈希和一名主编快照；应用层保证每本书最多发送一次该类模型请求。`error_code + failure_stage + retry_safety` 是重试安全证据：只有明确的模型发送前条件失败才允许条件恢复后续跑，结果未知不得自动重发；成功条目和作者已确认版本始终保留。
...
- `0085_v7_planning_task_retries.sql`为规划维护任务追加`retry_count`；`0105_v7_planning_generation_retries.sql`为全书路线和规划树运行追加同一语义的尝试编号。只有明确`failed`可以增加尝试编号并交接；`working/unknown`保持冻结，避免重复调用和重复扣量。`0106_v7_setting_failure_recovery.sql`只为设定批次追加结构化失败阶段和重试安全性，不批量改写任何历史任务、输出或作者数据。旧会员门禁任务只在作者主动继续时，由服务同时核对工单、模型调用和成员事件后逐批恢复；无法确认是否已调用模型的旧任务保持冻结。
