...
`book_onboarding_profiles` 保存开书最小卡：创作频道，以及可选的一句话想法、主要标签和作品额外边界；书名仍在 `books`。界面只要求书名和频道。题材、目标读者、预计规模、初始表达基线、世界观、力量体系、核心冲突、主角、主线、结局和具体技法不作为建书必填字段，后续通过讨论候选与正式规划版本保存。字段来源标记为老板明确、系统建议、未指定或冲突。当前UI预览复用既有定位字段和标签绑定；软/硬/候选的独立运行时生命周期在老板确认界面后再通过向前迁移实现，不能用名称前缀冒充最终权威模型。
