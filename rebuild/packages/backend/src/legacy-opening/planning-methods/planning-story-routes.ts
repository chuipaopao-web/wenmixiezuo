...
import type { V7PlanningRouteVolume, V7PlanningStoryRoute } from './planning-route-contracts.js';
export type { V7PlanningRouteVolume, V7PlanningStoryRoute } from './planning-route-contracts.js';
...
export function planningDirectStoryRoutePrompt(input: {
  ...
  return [
    ...
    '你必须在一次工作中同时完成：按需选择本层方法或自行组合原创，不凑方法数量、设计完整的全书粗路线和各卷责任，并写清自己的设计理由。',
...
export function planningDirectStoryRouteRepairPrompt(input: {
  ...
  return [
    ...
    `brief.schema必须是v7-progressive-planning-brief-v2，brief.seatKey必须是${input.seatKey}；selectedStrategies按需填写0—6项，原创标agent_original。`,
...
export function planningRouteFusionPrompt(input: {
  ...
  return [
    ...
    '最终路线与精简设计依据必须彼此一致。selectedStrategies按需填写0—6项，不要求凑数量；库方法只能引用本次候选，原创策略只属于当前书。',
