...
describe('V7 作者端账号 API', () => {
  ...
  it('本地重构 API 尚未实现 auth/me 时显示可重试的暂时不可用', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({ error: { message: 'route not found' } }, 404)));

    await expect(fetchCurrentAuthorAccount()).rejects.toMatchObject({
