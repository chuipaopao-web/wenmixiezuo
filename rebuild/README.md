...
Batch 123 adds the internal PostgreSQL usage reservation core. It preserves the legacy compute conversion (real tokens × 2), immutable entitlement sources and usage events, owner-serialized reservations, idempotent settlement and release, provider-call uniqueness and unknown/over-budget protection. Caller transactions can include the reservation. Only explicit not-started evidence can release a pending reservation; recorded actual over-budget usage remains under review and cannot be overwritten by a lower settlement. Overlapping entitlement snapshots are rejected until an explicit amendment workflow exists. Full verification passes 318 tests plus 9 independent compiled PostgreSQL checks. Local dev has migrations 0008/0009. No public quota-grant API, real membership migration, supplier invoice ledger, parent-task attempt allocation, model execution or production cutover is included. See [batch 123](../coauthoring-v7/docs/worklists/REBUILD-USAGE-CORE-20260906-123.md).

Batch 122 adds the authenticated readonly editorial directory with the preserved 23 public members in 7 departments. Identity/name/role parity is verified against the legacy registry. Generation is not connected: all members are explicitly unavailable for new tasks, with no fabricated work history. Full rebuild verification passes 303 tests. The production release in this batch is the existing V7 admin roadmap only; this independent PostgreSQL backend has not been deployed. Next dependencies are usage reservation, real opening tasks and recovery, model execution and candidate confirmation, plus account/entitlement/legacy-book compatibility before author cutover. See [batch 122](../coauthoring-v7/docs/worklists/REBUILD-CONTINUATION-20260906-122.md).
