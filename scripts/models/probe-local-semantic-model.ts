...
import { LocalSemanticUtilityModel } from '../../apps/api/src/infrastructure/retrieval/local-semantic-utility-model.js';
...
if (modelPath === undefined) throw new Error('LOCAL_SEMANTIC_MODEL_NOT_INSTALLED');
...
if (hashDirectory(modelPath) !== manifest.assetHash) throw new Error('LOCAL_SEMANTIC_MODEL_HASH_MISMATCH');
...
const documents = await embedding.embedDocuments(['张三准备对天安城宣战', '李四在河岸清点粮草', '今夜月色很安静']);
const scores = documents.map((vector) => dot(query, vector));
if (query.length !== 512 || scores[0]! <= scores[1]! || scores[0]! <= scores[2]!) throw new Error('LOCAL_SEMANTIC_PROBE_FAILED');
const intent = await new LocalSemanticUtilityModel(embedding).infer({
  task: 'intent_classification', text: '咱们一起盘一盘，张三接下来该不该攻城？'
});
if (intent.values.intent !== 'plot_discussion' || intent.confidence < 0.58) {
  throw new Error(`LOCAL_SEMANTIC_INTENT_PROBE_FAILED:${JSON.stringify({ values: intent.values, confidence: intent.confidence })}`);
}
process.stdout.write(`${JSON.stringify({
  ...
  intent: { ...intent.values, confidence: intent.confidence },
