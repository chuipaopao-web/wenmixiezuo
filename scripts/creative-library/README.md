...
`build-emotional.ts`生成修订计划；`apply-emotional.mjs <source> <db> <emotional.json> <sha256> <review-final-one.json> preview|apply`在一个同步事务中核对来源、变更、审核、发布。先preview再apply，重复apply应零变更。遇后台人工修改、审核hash不符、其他活动版本或关系冲突即停止，不能覆盖或伪造审核。

逐条审核证据在`evidence/r209-c4/`：initial→corrected→final-one保留原始通过证据和按identity+payloadHash复用的关系。仅含公共创作方法和虚构样例，无作者资料、密钥或思维链。真实模型probe使用既有套餐，结果未知不自动重发。

运行入口为`apps/api/src/application/creative-reference/runtime.ts`，0124表存冻结版本和短工具动作。开书、设定、故事线、全书方向已接；每次最多6轮/3项。先按用途及阶段查短摘要，read完整详情，再ready选定并说明本书用法。查询零命中可参考明确标识的同用途候选或原创。最终只注入选中项，完整模型输入<=15000字符。已有独立审查继续执行，不增加固定“思考等待”。

公共模型探针结果证明工具执行与引用，不证明所有作品的文学质量；修订样例仍有“人物底色不变”等无来源断言风险，需独立审查对照原文。GLM一次创作超时已记录为unknown，不能算成功。

## C3最新状态（2026-09-14）

349现用方法逐项精修，新增12条，共361方法草稿；5条退役来源和196参考不改。保留稳定编号与版本证据，未独立审核、未发布为AI正式供给。

- `refine-methods.ts`按明确编号记录用途、类型、重点阶段和条件阶段判断，不是关键词批量分类。34个用途子类有覆盖，不代表穷尽全部文学方法。
- `build-refinement.ts`验证冻结C2计划及349项覆盖，生成`generated/refinement.json`与sha256。当前hash为`a94d63e2c86a6655a17873926921a2fc717d635f23efbe95b3f8506db5639c68`。
- `apply-refinement.mjs <release-source> <db> <plan> <sha256> preview|apply`走现有管理事务，校验预期revision及原内容；冲突整批回滚，重复执行零新增/零修改，实际编号映射保留旧别名。
- `verify-refinement.mjs <release-source> <db>`核对真实仓储用途查询、重点阶段过滤和库完整性。用途匹配主用途或关联用途，阶段筛选只匹配重点阶段；条件阶段需判断触发条件，不能全部装入上下文。
- 新增12条：成果可见化、回报错峰、回报方式换新、期待进展可辨、融合主次取舍、事实证据对照、人物知情核查、状态条件核查、时间因果核查、重复信息修订、人物声音修订、因果缺口核查。
- 29项数据/接口测试、17项管理端测试、两端类型检查和构建通过。生产证据位于`/opt/wenmi-releases/wm-v7-20260914-001000-c3000001/`的applied/repeated/content-verified.json。浏览器连接超时，视觉点击未完成。D/E运行接入仍待开发。
