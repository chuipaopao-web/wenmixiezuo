...
if(plan.batch!=='r209-c6'||plan.entries.length!==33||plan.additions.length!==32||report.model!=='glm-5.3')throw Error('修订合同不符');
const evidence=[...(report.reused??[]),...report.batches.flatMap(b=>b.state==='done'?b.items:[])];
if(evidence.length!==594||new Set(evidence.map(e=>e.identity)).size!==594||evidence.some(e=>!e.pass||e.issues.length))throw Error('独立审核未全部通过');
...
const db=new DatabaseSync(path),repo=new SqliteCreativeReferenceRepository(db),audit=new CreativeReferenceAdminRepository(db),actor={role:'manager',actorId:'codex-r209-c6'},service=new CreativeReferenceAdminService(repo,audit,{canManage:a=>['codex-r209-c6','glm-5.3:r209-c6'].includes(a.actorId)}),key='r209-c6-'+sha(plan),preview=Symbol();let result;
try{repo.runInTransaction(()=>{
 ...
 for(const addition of plan.additions){validatePayload(addition.payload);service.createCardWithAudit({payload:addition.payload,idempotencyKey:addition.identity,legacy:{namespace:'r209-c6',key:addition.sourceCode,version:1}},actor,new Date().toISOString());}
 const entries=[];for(const row of db.prepare("SELECT internal_id,idempotency_key identity FROM creative_reference_cards WHERE status<>'retired'").all()){
  ...
  if(r.status==='draft')service.reviewWithOpinion(c.internalId,r.revision,'C6链检查范围审核通过；内容哈希 '+v.payloadHash,{role:'manager',actorId:'glm-5.3:r209-c6'},new Date().toISOString());entries.push({internalId:c.internalId,revision:r.revision});
 }
 if(entries.length!==594)throw Error('全库数量已变化');
 ...
 result={changed:33,added:32,total:594,releaseId:published.releaseId,mode};if(mode==='preview')throw preview;
