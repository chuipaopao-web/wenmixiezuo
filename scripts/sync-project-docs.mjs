...
const currentPaths = [
  ...
  'docs/CREATION_WORKFLOW_V2_DESIGN.md', 'docs/ARCHITECTURE.md', 'docs/DATA_MODEL.md',
...
const generatedPaths = new Set(['docs/PROJECT_DOCUMENT_INDEX.md', 'docs/PROJECT_DOCUMENT_CENTER.html']);
...
const templates = documents.map((item) => `<template id="${item.id}" data-title="${htmlEscape(item.title)}" data-path="${htmlEscape(item.path)}" data-meta="${htmlEscape(`${item.category} · ${formatBytes(item.bytes)} · ${item.hash}`)}">${markdownToHtml(item.content, item.path)}</template>`).join('');
...
if (checkOnly && (!markdownCurrent || !htmlCurrent)) {
