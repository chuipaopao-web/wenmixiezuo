...
const xianxia = makeScenario({
  ...
  openingBlueprint(taxonomyVersion) {
    return {
      ...
      mainTags: ['仙侠', '修仙', '逆袭', '冒险'],
      auxiliaryTags: ['阵法禁制', '剑修', '灵根', '宗门学院', '证据链'],
      storyTraits: ['越级战斗', '宗门成长', '智斗'],
      styleIntent: {
  ...
  answers: {
    levels: '修行主阶从低到高为淬体、引气、凝脉、筑台、金丹、元婴、化神，每境分九重。淬体锻骨开脉，引气纳灵入体，凝脉贯通十二正脉，筑台形成可稳定承载神识与阵势的灵台，金丹把灵力凝成自循环核心，元婴使神魂能够离体御阵，化神则以神意影响天地灵机。小境晋升需要身体、灵力和技法积累；跨大境还必须完成本境能力验证、稳定心境并取得与自身相容的资源，缺一不可。沈砚灵根驳杂，吸收与转化更慢、属性冲突更重；残阵盘和阵纹只能提高理解、借用环境或减少试错，不能替代修为积累。越级借阵会形成可追踪的阵痕，轻则经脉灼痛、灵力失衡，重则留下裂痕并提高下一次突破难度；阵痕只能靠休养、对症资源和逐步修复减轻，不能一夜清零。',
...
const esports = makeScenario({
  ...
  openingBlueprint(taxonomyVersion) {
    return {
      ...
      mainTags: ['游戏', '竞技', '逆袭', '群像'],
      auxiliaryTags: ['电子竞技', '公会战队', '联赛规则', '赛季', '排行榜'],
      storyTraits: ['智斗', '职业选手', '赛事成长'],
      styleIntent: {
