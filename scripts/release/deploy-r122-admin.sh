...
[[ $# == 3 ]] || { echo 'usage: deploy-r122-admin.sh fix8 archive-sha stage|cutover' >&2; exit 64; }
FIX=$1
SHA=$2
MODE=$3
[[ $FIX =~ ^[a-f0-9]{8}$ && $SHA =~ ^[a-f0-9]{64}$ && $MODE =~ ^(stage|cutover)$ && $EUID == 0 ]] || exit 64

BASE=wm-v7-20260906-105500-58aa0e75
NEW=wm-v7-20260906-125500-$FIX
ROOT=/opt/wenmi-releases/$NEW
SRC=$ROOT/source
OLD=/opt/wenmi-releases/$BASE/source
DB=/opt/wenmi/data/database/wenmi.sqlite
STATIC_OLD=/opt/wenmi/releases/versions/4f6d09ded6793ff2f998
ARCHIVE=/tmp/r122-$FIX-built.tar.gz
...
atomic_link() { rm -f "$2.r122-next"; ln -s "$1" "$2.r122-next"; mv -Tf "$2.r122-next" "$2"; }
switch_static() { rm -f /opt/wenmi/releases/.r122-next; ln -s "$1" /opt/wenmi/releases/.r122-next; mv -Tf /opt/wenmi/releases/.r122-next /opt/wenmi/releases/current; }
write_release() {
  printf '%s\n' "$1" >/opt/wenmi/RELEASE_ID.r122-next
  chown wenmi:wenmi /opt/wenmi/RELEASE_ID.r122-next
  chmod 644 /opt/wenmi/RELEASE_ID.r122-next
  mv -Tf /opt/wenmi/RELEASE_ID.r122-next /opt/wenmi/RELEASE_ID
}
api_ready() {
...
publish_plan() {
  ...
  install -o wenmi -g wenmi -m 644 "$SRC/docs/REBUILD_EXECUTION_PLAN.md" "$PLAN_TARGET.r122-next"
  mv -Tf "$PLAN_TARGET.r122-next" "$PLAN_TARGET"
}
restore_plan() {
  if [[ -f "$ROOT/plan-before-state" && $(cat "$ROOT/plan-before-state") == present ]]; then
    install -o wenmi -g wenmi -m 644 "$ROOT/plan-before" "$PLAN_TARGET.r122-rollback"
    mv -Tf "$PLAN_TARGET.r122-rollback" "$PLAN_TARGET"
  else
    rm -f "$PLAN_TARGET"
  fi
}
verify_author_static_unchanged() {
...
def author_files(root):
    ...
    for p in root.rglob('*'):
        ...
        if rel=='release-manifest.json' or rel.startswith('v7/'): continue
        files[rel]=p
    return files
old_files=author_files(old); target_files=author_files(target)
if set(old_files)!=set(target_files):
    raise SystemExit('author file set changed')
for rel,p in old_files.items():
    if not filecmp.cmp(p,target_files[rel],shallow=False):
        raise SystemExit(f'author file changed: {rel}')
PY
}
verify_old_admin_assets_preserved() {
  python3 - "$STATIC_OLD" "$1" <<'PY'
import hashlib,pathlib,sys
old,target=map(pathlib.Path,sys.argv[1:3])
for p in (old/'v7').rglob('*'):
    if not p.is_file(): continue
    rel=p.relative_to(old/'v7').as_posix()
    if rel=='index.html': continue
    if not (target/'v7'/rel).is_file():
        raise SystemExit(f'old admin asset missing: {rel}')
    if hashlib.sha256(p.read_bytes()).digest()!=hashlib.sha256((target/'v7'/rel).read_bytes()).digest():
        raise SystemExit(f'old admin asset changed: {rel}')
PY
}
...
PY
  ...
  node node_modules/tsx/dist/cli.mjs scripts/quality/verify-v7-runtime-source-closure.ts --output artifacts/r122-closure.json >"$ROOT/closure.log"
  node node_modules/tsx/dist/cli.mjs scripts/release/verify-v7-release-module-resolution.ts --release-source "$SRC" --manifest "$SRC/artifacts/r122-closure.json" >"$ROOT/modules.log"
  node node_modules/vitest/vitest.mjs run --configLoader native tests/integration/security/rebuild-control.test.ts tests/integration/security/v7-admin-console.test.ts >"$ROOT/security-tests.log" 2>&1
  ...
  R122_DB="$ROOT/preflight/wenmi.sqlite" node --input-type=module <<'JS' >"$ROOT/migrations.json"
import {DatabaseSync} from 'node:sqlite';
import {runMigrations} from './apps/api/dist/infrastructure/db/migrations.js';
const db=new DatabaseSync(process.env.R122_DB);db.exec('PRAGMA foreign_keys=ON');
...
PY
  R122_STATIC_ROOT="$ROOT/static-input" R122_SOURCE="$SRC" node --input-type=module <<'JS' >"$ROOT/static.json"
const {assembleV7StaticRelease}=await import(process.env.R122_SOURCE+'/scripts/release/v7-static-release.mjs');
console.log(JSON.stringify(await assembleV7StaticRelease({projectRoot:process.env.R122_STATIC_ROOT,authorDist:'author',adminDist:'admin'})));
JS
  ...
  verify_author_static_unchanged "$TARGET"
  verify_old_admin_assets_preserved "$TARGET"
  printf '%s\n' "$ID" >"$ROOT/static-id"
  remember_plan_state
  touch "$ROOT/stage-passed"
  echo "R122_STAGED release=$NEW static=$ID backup=$backup"
  exit 0
fi
...
if ! wait_zero_window predeploy 300; then
  echo "R122_NOT_DEPLOYED active_tasks_timeout" | tee "$ROOT/not-deployed"
  exit 75
fi
...
rollback() {
  ...
  if wait_zero_window rollback 300; then
    ...
    health "$BASE"
    echo "R122_ROLLED_BACK code=$code"
  else
    echo "R122_ROLLBACK_DEFERRED active_work" >"$ROOT/rollback-deferred"
  fi
  exit "$code"
}
trap rollback ERR
...
ln -s "$STATIC_OLD" /opt/wenmi/releases/.r122-previous
mv -Tf /opt/wenmi/releases/.r122-previous /opt/wenmi/releases/previous
...
with urllib.request.urlopen('https://wenmixiezuo.com/',timeout=30) as r:
    assert r.read()==(old/'index.html').read_bytes(),'author homepage changed'
...
echo "R122_DEPLOYED release=$NEW static=$ID worker_ready"
