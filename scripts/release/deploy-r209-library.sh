...
PY
 ...
 sqlite3 "$DB" ".backup '$ROOT/preflight/before.sqlite'"
...
rollback(){ local code=$?; trap - ERR; if zero; then systemctl stop wenmi-api; link "$OLD/apps" /opt/wenmi/apps; release "$BASE"; link "$STATIC_OLD" /opt/wenmi/releases/current; systemctl reset-failed wenmi-api; systemctl start wenmi-api; ready "$BASE" 43111; systemctl start wenmi-worker; echo ROLLED_BACK; else echo ROLLBACK_DEFERRED_ACTIVE_TASKS; fi; exit "$code"; }
trap rollback ERR
[[ $(count) == 0 ]]
sqlite3 "$DB" ".backup '$ROOT/preflight/pre-cutover.sqlite'"
...
assert any(0 <= (datetime.datetime.now(datetime.timezone.utc)-datetime.datetime.fromisoformat(t.replace('Z','+00:00'))).total_seconds()<20 for (t,) in rows)
PY
