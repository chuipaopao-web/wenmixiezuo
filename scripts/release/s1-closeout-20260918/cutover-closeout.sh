#!/bin/bash
set -Eeuo pipefail
ROOT=/opt/wenmi-releases/wm-v7-20260918-161500-e1310473
SRC=$ROOT/source
RB=$ROOT/rollback/source
OLD=wm-v7-20260914-151504-a7614958
NEW=wm-v7-20260918-161500-e1310473
DB=/opt/wenmi/data/database/wenmi.sqlite
STATIC_OLD=/opt/wenmi/releases/versions/fbbb1a6993a21c1aab07
STATIC_NEW=/opt/wenmi/releases/versions/8612537aae8d01aab4d6
exec 9>/opt/wenmi-releases/.deploy.lock
flock -n 9
test "$(cat /opt/wenmi/RELEASE_ID)" = "$OLD"
test "$(readlink -f /opt/wenmi/apps)" = "/opt/wenmi-releases/$OLD/source/apps"
test "$(readlink -f /opt/wenmi/releases/current)" = "$STATIC_OLD"
test -f "$ROOT/rollback-migrate.json"
test -f "$ROOT/precutover-tests-passed"
test -f "$ROOT/channel-passed"
...
# Verify backup checksums independently before the maintenance window.
...
with urllib.request.urlopen('https://wenmixiezuo.com/assets/index-DzlVU_Da.js',timeout=20) as r:assert r.status==200
print(json.dumps({'release':sys.argv[2],'static':m['releaseId'],'assets':len(m['files']),'protectedRoutes':4,'oldAssets':'200'}))
PY
trap - ERR INT TERM
