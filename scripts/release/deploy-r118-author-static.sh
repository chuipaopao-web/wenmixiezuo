...
set -Eeuo pipefail
umask 077
[[ $# == 3 ]] || { echo 'usage: deploy-r118-static.sh commit archive-sha stage|activate'; exit 64; }
FIX=$1
SHA=$2
MODE=$3
...
OLD=/opt/wenmi/releases/versions/32a51662b5b21ea71144
SOURCE=/opt/wenmi-releases/$BASE/source
ROOT=/opt/wenmi-releases/r118-navigation-$FIX
ARCHIVE=/tmp/r118-navigation-$FIX.tar.gz
[[ $EUID == 0 ]] || exit 64
...
PY
 R118_ROOT="$ROOT" R118_SOURCE="$SOURCE" R118_OLD="$OLD" node --input-type=module <<'JS' >"$ROOT/static.json"
const {assembleV7StaticRelease}=await import(process.env.R118_SOURCE+'/scripts/release/v7-static-release.mjs');
console.log(JSON.stringify(await assembleV7StaticRelease({projectRoot:process.env.R118_ROOT,authorDist:'author',adminDist:process.env.R118_OLD+'/v7'})));
JS
...
switch_static() { ln -s "$1" /opt/wenmi/releases/.r118-next; mv -Tf /opt/wenmi/releases/.r118-next /opt/wenmi/releases/current; }
rollback() { code=$?; trap - ERR; switch_static "$OLD"; echo "R118_ROLLED_BACK exit=$code" >&2; exit "$code"; }
trap rollback ERR
...
ln -s "$OLD" /opt/wenmi/releases/.r118-previous
mv -Tf /opt/wenmi/releases/.r118-previous /opt/wenmi/releases/previous
trap - ERR
printf 'static=%s\ncommit=%s\ncompleted=%s\napiPid=%s\nworkerPid=%s\n' "$ID" "$FIX" "$(date -u --iso-8601=seconds)" "$API_PID" "$WORKER_PID" >"$ROOT/deployment-passed.txt"
echo "R118_DEPLOYED static=$ID api_unchanged=$API_PID worker_unchanged=$WORKER_PID"
