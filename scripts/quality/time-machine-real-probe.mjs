// Runs only against a temporary in-memory database. Credentials stay in the process environment.
...
const db=new DatabaseSync(':memory:');db.exec('PRAGMA foreign_keys=ON');runMigrations(db,resolve(root,'migrations'));
const {AUDITED_RUNTIME_POLICY}=await moduleAt('node_modules/@wenmi/v7-backend/dist/index.js');
if(AUDITED_RUNTIME_POLICY)db.prepare('INSERT INTO v7_rhythm_policy_versions VALUES(1,?,?,?)').run(JSON.stringify(AUDITED_RUNTIME_POLICY),'probe',new Date().toISOString());
const scope={ownerId:'tm2-probe-owner',bookId:'tm2-probe-book'},now=new Date().toISOString();
db.prepare('INSERT INTO owners VALUES(?,?,1,?,?)').run(scope.ownerId,'隔离测试',now,now);
new BookRepository(db).create(scope,'机甲修理工·隔离测试',now,'active');
...
db.prepare("INSERT INTO book_opening_blueprints VALUES('opening',?,?,1,'v1','male','fantasy','玄幻',?,?,'active',?)").run(scope.ownerId,scope.bookId,JSON.stringify(source),'a'.repeat(64),now);
...
try{
 const recommendation=service.start(scope,'recommend','','probe-recommend');await service.process(recommendation);
 const design=service.start(scope,'design','单主线；成长线、机甲伙伴线；兼顾伙伴自身选择','probe-design');await service.process(design);
 const runs=service.state(scope),usage=accountUsageTotals(db,{ownerId:scope.ownerId});
 let handoff=null;
 const designed=runs.find(r=>r.kind==='design');
 ...
 writeFileSync(resolve(root,'result.json'),JSON.stringify({runs,usage,diagnostic,calls,handoff},null,2),{mode:0o600});
 ...
 if(runs.some(r=>r.state!=='succeeded')||!handoff)process.exitCode=2;
}finally{db.close();}
