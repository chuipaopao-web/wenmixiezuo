...
// K3批：骨架/卷卡夹具按提示词中的“作者已确认故事线”清单动态生成covers与职责，适配任意合法选择。
let designLineIds:string[]=['main'];
function output(prompt:string):unknown{
 ...
 if(prompt.includes('设计全书骨架。只设计')){
  const required=/不得丢弃、合并或改名：(.+?)。每条作者故事线/u.exec(prompt)?.[1]?.split('、')??['成长线'];
  const lines=required.map((title,i)=>({id:i===0?'main':`line${i+1}`,role:i===0?'main':'through',title:i===0?'工坊':`支线${i+1}`,goal:'立足',answer:'建立工坊',process:'从修理到建坊',parentIds:[],covers:[title],milestones:[]}));
  designLineIds=lines.map(l=>l.id);
