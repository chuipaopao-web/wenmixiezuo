...
describe('new time machine session boundary',()=>{
 it('authenticates new prefix, isolates books and enforces browser write origin',async()=>{
  ...
  try{
   ...
   const response=await app.inject({url:'/api/time-machine/books/tm-book/state',headers:{...headers,cookie}});expect(response.statusCode).toBe(200);expect(response.json().data).toMatchObject({enabled:false,runs:[],adopted:null,planRevision:0,preparation:{ready:false}});
   ...
   expect((await app.inject({method:'POST',url:'/api/time-machine/books/tm-book/design-runs',headers:{...headers,cookie},payload:{idempotencyKey:123}})).statusCode).toBe(409);
 ...
 it('saves manual candidate revisions with expectedRevision and keeps old ones',async()=>{
  ...
  try{
   ...
   const manifest=sampleV2().manifest;manifest.sources[0]={kind:'opening',id:'opening',revision:'1',hash:digest({})};
   c.database.prepare("INSERT INTO book_opening_blueprints(opening_blueprint_id,owner_id,book_id,version,taxonomy_version,channel,category_key,category_name,blueprint_json,content_hash,status,created_at) VALUES(?,?,?,1,'test','male','test','测试','{}',?,'active',?)").run('opening-test',scope.ownerId,scope.bookId,digest({}),now);
   const snapshot={members:{writer:{memberKey:'writer-a',displayName:'红玉',model:{modelId:'model-x'},governanceRevision:3}},manifest,intent:'成长线'};
   ...
   c.database.prepare('UPDATE tm2_books SET manifest=? WHERE owner=? AND book=?').run(JSON.stringify(manifest),scope.ownerId,scope.bookId);
   repo.saveCandidate(scope,'cand-1',0,{schemaVersion:2,manifest,member:{id:'writer-a',name:'红玉',model:'model-x',routeRevision:'3'},plan});
   ...
   expect((await app.inject({method:'POST',url,headers:{...headers,cookie},payload:{plan:edited,expectedRevision:1}})).statusCode).toBe(409);
   // This test isolates revision/version security; prerequisite semantics have their own integration test.
   vi.spyOn(V7SettingEditorialService.prototype,'timeMachinePrerequisite').mockReturnValue({ready:true,message:'已确认',version:'test'});
   const saved=await app.inject({method:'POST',url,headers:{...headers,cookie},payload:{plan:edited,expectedRevision:1}});
   expect(saved.statusCode,saved.body).toBe(200);expect(saved.json().data).toEqual({revision:2});
