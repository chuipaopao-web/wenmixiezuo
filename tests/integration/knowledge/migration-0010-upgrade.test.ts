...
describe('Schema 9升级到当前表达、知识、对象协作与设定工作区', () => {
  it('只向前追加当前迁移，保留旧书并可重复执行', () => {
    ...
    try {
      ...
      expect(upgraded.applied.at(-1)).toBe('0113_method_agent_audit.sql');
      expect(upgraded.applied.slice(-32)).toEqual([
