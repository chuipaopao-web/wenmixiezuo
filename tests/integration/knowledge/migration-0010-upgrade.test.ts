...
describe('Schema 9升级到表达、知识生命周期与切片投影', () => {
  it('只向前追加0010—0012，保留旧书并可重复执行', () => {
    ...
    try {
      ...
      expect(upgraded.applied).toEqual(['0010_expression_taxonomy.sql', '0011_knowledge_lifecycle_time.sql', '0012_chunk_projection_snapshots.sql']);
