...
describe('阶段2已有数据升级', () => {
  it('保留阶段1发布记录并新增安全表', () => {
    const root = mkdtempSync(resolve(tmpdir(), 'wenmai-upgrade-'));
    ...
    try {
      ...
      database.prepare("INSERT INTO release_runs (release_id, product_name, schema_version, api_version, created_at) VALUES ('release-old', '文脉写作', 1, 'v1', '2026-07-16T00:00:00Z')").run();
