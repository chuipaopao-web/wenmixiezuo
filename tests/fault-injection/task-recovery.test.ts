...
describe('Worker失联与结果不明恢复', () => {
  it('存在working模型调用时标记interrupted且普通入队不能重试', () => {
    ...
    calls.begin(scope, { requestId: 'request-unknown', taskId: 'task-unknown', phaseKey: 'draft', agentId: agent.agentId, modelSnapshotId: snapshot.model_snapshot_id, provider: 'unknown', modelId: 'unknown', input: 'unknown', parameters: '{}', reservationId });
