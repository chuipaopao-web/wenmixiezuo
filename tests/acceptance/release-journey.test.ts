...
describe('首版全链路验收旅程', () => {
  ...
  it('两书严格隔离，主测试书中断接管后连续完成5章并可干净恢复', async () => {
    ...
    expect(new NarrativeProjectionService(context.database, ids, clock).rebuild(mainScope)).toBe(6);
    expect(context.database.prepare(`
      SELECT projection_type, track, COUNT(*) AS count
      FROM narrative_projections
      WHERE owner_id = ? AND book_id = ?
      GROUP BY projection_type, track
      ORDER BY projection_type, track
    `).all(ownerId, mainBook.bookId)).toEqual([
      { projection_type: 'hook', track: 'planned', count: 5 },
      { projection_type: 'mainline', track: 'planned', count: 1 }
    ]);
    expect(context.database.prepare(`
      ...
      WHERE owner_id = ? AND book_id = ?
        AND (
          projection_type IN ('subplot', 'information_gap')
          OR content_json LIKE '%endingExcerpt%'
        )
    `).get(ownerId, mainBook.bookId)).toEqual({ count: 0 });
    expect(context.database.prepare(`SELECT COUNT(*) AS count FROM narrative_projections WHERE owner_id = ? AND book_id = ?`).get(ownerId, secondBook.bookId)).toEqual({ count: 2 });
