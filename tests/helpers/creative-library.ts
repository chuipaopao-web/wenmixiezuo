...
export function seedCreativeLibrary(db:DatabaseSync){
 ...
 const card=repo.createCard({assetKind:'method',idempotencyKey:'test-payoff',legacy:null,authorActor:'test-designer',payload:{assetKind:'method',schemaVersion:1,name:'成果可见',shortPhrase:'成果可见',summary:'用实际变化表现回报',aliases:[],method:{title:'成果可见',instruction:'借角色行动与生活变化表现获得和失去',boundary:'不反复复述结果',usageTree:'阶段回报',relatedPurposes:['情绪回报'],applicableLayers:['opening','setting','book'],conditionalUses:[],aliases:[]}}},now);
