...
set -Eeuo pipefail
umask 077
BASE=wm-v7-20260905-193522-5285319f
NEW=wm-v7-20260905-203723-1eefc5c
ROOT=/opt/wenmi-releases/$NEW
SRC=$ROOT/source
OLD=/opt/wenmi-releases/$BASE/source
DB=/opt/wenmi/data/database/wenmi.sqlite
ARCHIVE=/tmp/r100-1eefc5c-built.tar.gz
SHA=3683cc059e8f12930bb9418f203cdff82b5dd2dab7c7ee84546c30012d01ab1a
STATIC_OLD=/opt/wenmi/releases/versions/c7402f64fd0a9818ef4f
fail() { echo "R100_FAILED: $*" >&2; exit 1; }
...
atomic_link() { ln -s "$1" "$2.r100-next"; mv -Tf "$2.r100-next" "$2"; }
write_release() { printf '%s\n' "$1" >/opt/wenmi/RELEASE_ID.r100-next; chmod 644 /opt/wenmi/RELEASE_ID.r100-next; chown wenmi:wenmi /opt/wenmi/RELEASE_ID.r100-next; mv -Tf /opt/wenmi/RELEASE_ID.r100-next /opt/wenmi/RELEASE_ID; }
active_count() {
 python3 - "$DB" <<'PY'
import sqlite3,sys
...
PY
 ...
 cmp "$SRC/package-lock.json" "$OLD/package-lock.json"
 cd "$SRC"
 node node_modules/tsx/dist/cli.mjs scripts/quality/verify-v7-runtime-source-closure.ts --output "$SRC/artifacts/deploy/closure-r100.json" >"$ROOT/closure.log"
 node node_modules/tsx/dist/cli.mjs scripts/release/verify-v7-release-module-resolution.ts --release-source "$SRC" --manifest "$SRC/artifacts/deploy/closure-r100.json" >"$ROOT/modules.log"
...
rows=d.execute('SELECT name,checksum FROM schema_migrations').fetchall(); assert len(rows)==106
...
print('existing_106_migrations_unchanged')
PY
...
PY
 R100_SOURCE="$SRC" R100_ADMIN="$STATIC_OLD/v7" node --input-type=module <<'JS' >"$ROOT/static.json"
import {assembleV7StaticRelease} from './scripts/release/v7-static-release.mjs';
console.log(JSON.stringify(await assembleV7StaticRelease({projectRoot:process.env.R100_SOURCE,adminDist:process.env.R100_ADMIN})));
JS
 ...
 R100_SOURCE="$SRC" R100_DB="$ROOT/preflight/database/wenmi.sqlite" node --input-type=module <<'JS' >"$ROOT/migration-preflight.json"
import {DatabaseSync} from 'node:sqlite';
import {runMigrations} from './apps/api/dist/infrastructure/db/migrations.js';
const db=new DatabaseSync(process.env.R100_DB);db.exec('PRAGMA foreign_keys=ON');
const before=db.prepare("SELECT name,sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' ORDER BY name").all();
const first=runMigrations(db,process.env.R100_SOURCE+'/apps/api/src/infrastructure/db/migrations');
const second=runMigrations(db,process.env.R100_SOURCE+'/apps/api/src/infrastructure/db/migrations');
if(JSON.stringify(first.applied)!=='["0107_v7_route_decision_jobs.sql"]'||second.applied.length)throw Error('unexpected migrations');
...
JS
 ...
 sudo -u wenmi env -i PATH=/usr/local/bin:/usr/bin:/bin HOME=/opt/wenmi USER=wenmi LANG=C.UTF-8 \
  ...
  WENMI_WORKER_TOKEN=r100-isolated-no-model timeout 60 node "$SRC/apps/api/dist/main.js" >"$ROOT/isolated.log" 2>&1 &
 isolated_pid=$!
 isolated_ok=0
...
cd "$SRC"
sudo -u wenmi env R100_SOURCE="$SRC" R100_DB="$DB" node --input-type=module <<'JS' >"$ROOT/migration-production.json"
import {DatabaseSync} from 'node:sqlite';import {runMigrations} from './apps/api/dist/infrastructure/db/migrations.js';
const db=new DatabaseSync(process.env.R100_DB);db.exec('PRAGMA foreign_keys=ON');
const result=runMigrations(db,process.env.R100_SOURCE+'/apps/api/src/infrastructure/db/migrations');
if(JSON.stringify(result.applied)!=='["0107_v7_route_decision_jobs.sql"]')throw Error('unexpected migration');
