...
- 明确不改：API、数据库、注册登录语义、会员语义、后台代码、部署脚本、生产服务器、真实作者数据。
...
- 风险与限制：
  ...
  - 本批没有修改 API、数据库、注册登录语义、会员语义、后台代码或部署脚本。
...
- 发布准备：仅作者端静态文件及对应源码，复用线上后台原文件与旧哈希资源；主代理发布脚本`artifacts/deploy/deploy-r105-static.sh`不会重启API/Worker或写数据库。发布成功后补齐版本和公网验证记录。
