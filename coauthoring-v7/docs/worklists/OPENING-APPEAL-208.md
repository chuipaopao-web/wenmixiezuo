...
状态：GLM返修后经Codex复核，2026-09-13已整合并发布R208字段补全。工程与页面通过；真实合成日常样例目录校验失败仍待修，详见第13节。本文件是本批唯一实施合同。
...
## 12. Codex首轮审计与限定返修（2026-09-13）

审计结论：暂不通过上线验收。字段持久化主要链路已实现，不需要重写；只补以下缺口。继续使用原隔离副本、原分支，禁止重新创建项目、清空差异或触碰生产。

### 必须修复

1. **旧资料任务恢复输入不兼容（高优先级）。** `time-machine-card-template.ts`由card-5切card-6，同时修改了全局cardContract；`time-machine-design-service.ts`仍对所有快照无条件使用新合同，并原位修改`card-review:${audit}:${i}`提示。`StepRepository.create`对同一步骤核对完整输入hash，改变prompt会抛“接续不能改变输入或成员”，即使已有成功输出也先执行此校验。只递增manifest模板号无法保护已冻结旧任务。按snapshot.manifest.templateRevision选择合同及review提示：旧card-5（含此前快照）使用R207原字符串，新card-6使用新提示。覆盖card页、card-finalize及card-review和它们的repair；不得覆盖旧步骤、重写快照或靠新节点强迫旧任务重做。对旧快照预置成功页/暂时失败review后续跑提供服务级回归，校验既有页复用、输入hash不变、新版本包含两字段职责。
2. **阅读味道审查输出链路缺口。** 活跃及兼容`opening-prompt-compiler.ts`的review decisions.field enum遗漏`positioning.readingTone`，虽然API返修白名单已添加，模型仍不能按schema提出该字段的决定。补齐enum、检查审查解析/字段映射/返修保护的全部相关入口；`NewNovelPage.tsx`的REVIEW_FIELD_LABELS新增“阅读味道”，原coreAppeal统一为“核心卖点”。本次允许增加该文件及直接测试。测试实际review schema包含该字段、作者采纳该项可以进入返修、只修改味道不改未授权字段、前端不出现内部路径。
3. **输入规格未完成。** 新增两项使用ImeInput，是单行input，并非约定2—3行文字框。更改为局部多行输入，手机可阅读和编辑完整短句。现有ImeInput/ImeTextarea会用slice截断超长粘贴；新字段不能复用这一静默丢字行为。为本次两字段保留完整输入、显示中文超限错误，阻止无效保存并保留内容，不修改其他字段全局行为。测试中文输入法、粘贴801/301字符不丢文本、修短可保存；补390px编辑态截图。

### 补足验证及交付证据

- A4不能仅以长度校验替代：增加资料保存失败、expectedVersion冲突时保留输入、不假报成功的交互测试。
- 首轮报告A1名称写“确认建书→读资料”，但首个用例只做转换函数，A8用例才实际建书读取；A2测试用例只比较对象。报告准确对应证据，别用相同对象比较替代浏览器改名/保存证明。
- 两份compiler忽略行尾后内容一致。当前diff有大量CRLF尾空白报警；仅对本批已修改文件恢复仓库既有换行规范，不格式化无关文件。最终提供`git diff --check`结果。
- UI证据写出截图的绝对路径，覆盖信息页及打开编辑后的两字段；不要只写“artifacts会话目录”。真实模型验证仍交Codex，本次不自行调用生产。
- 后台记录标为“首轮审计发现缺口，返修后待复核”，不得写Codex已验收。不要处理首轮报告的两项无关基线失败，单独保留待办。

### Codex已复验

在隔离副本重新构建opening-runtime和v7-backend后，`opening-appeal-208.test.ts`及`v7-opening-confirm-consistency.test.ts`共12项通过。初次未重建时两项readingTone返回undefined，重建后消失，归为旧构建产物，不作为源码缺陷。确认依赖运行产物必须先构建再测，报告应给出可重复顺序。

返修完成覆盖更新原`outbox/task-208.result.md`，追加逐项修复位置、测试命令与截图路径；保持未提交、未合入、未部署，由老板手动通知Codex复核。

## 13. Codex第二轮复核与发布

- GLM完成三项返修；Codex复读差异并重建runtime/backend，5套后端40项与2套前端65项通过，共105项。主目录API类型检查、13项功能档案摘要核对及diff检查通过。
- 390px本地真实页面打开两项多行输入，修改阅读味道后保存、刷新仍在；卖点、书名、其他资料保持。GLM的edit-390截图只拍到弹窗上半段，不能当作输入区域截图；Codex已用浏览器实看补验。未在生产编辑作者资料。
- 旧card-5合同与R207源字符串对照相同，旧审查提示保持同串。服务回归覆盖成功节点复用；未模拟真实网络中断后恢复，不扩大证据结论。
- 三组合成想法使用既有Coding Plan/DeepSeek Flash，不读取作者数据、不修改生产任务。三国21.1秒、悬疑21.4秒完整解析通过；日常首次4.9秒、纠错复测14.4秒均因目录外标签“温暖”失败。三者核心卖点/阅读味道均有输出；日常保持温暖、无外挂、不写战争，但完整生成不能记通过。纠错探针补了错误说明，未完整运行引擎自修循环；后续需定位真实引擎目录约束与修复指令，不能据此直接认定引擎恢复失败。未追加第三次调用、未放宽标签校验。
- API/Worker：wm-v7-20260913-053000-a2080001；静态976dd329934d83ef1f77。独立目录构建，14静态文件manifest核验、13后台档案核对通过；切换前在途连续30秒为零并立即复核，未取消作者任务。保留旧资源与R207回滚版本，无迁移、无数据恢复。切换后API status=ok、Worker ready，作者首页入口对比、后台入口可达、未认证任务API 401。
- 后台只更新本次功能说明/代码摘要/实施记录的窄范围元数据；未上传完整内部路线或作者资料。题材创作思路库、跨层自动失效仍未实现；两项先前已有测试失败保持独立待办。本批不是整个时光机或全产品完成。
