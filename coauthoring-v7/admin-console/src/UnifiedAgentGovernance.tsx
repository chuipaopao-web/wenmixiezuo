...
function MemberCard({ data, role, member, busy, update }: {
  ...
  const canReturn = member.admission === undefined || member.admission.status === 'compatible';
  return <article className={`agent-member-card ${member.enabled ? 'enabled' : 'disabled'}`}>
    <div className="agent-member-identity"><span className="agent-avatar" aria-hidden="true" style={{ backgroundImage: `url('${V7_MEMBER_AVATAR_SPRITE}')`, backgroundSize: V7_MEMBER_AVATAR_SIZE, backgroundPosition: publicMemberIdentity(member.memberKey)?.avatarPosition ?? '100% 100%', flexShrink: 0 }}/><div><h3>{publicMemberIdentity(member.memberKey)?.displayName ?? member.displayName}</h3><p>{member.modelName} · {member.plan === 'image' ? '图片' : member.plan === 'agent' ? 'Agent Plan' : 'Coding Plan'}</p></div><span className={`agent-duty-state ${member.status === 'on_duty' ? 'on' : 'off'}`}>{member.status === 'on_duty' ? '在岗' : '请假'}</span></div>
    <div className="agent-member-order"><label><span>绑定模型</span><select value={member.modelProfileKey} disabled={busy} onChange={(e) => void update(member.memberKey,{modelProfileKey:e.target.value},`已调整${member.displayName}的模型`)}>{candidates.map((candidate) => <option key={candidate.profileKey} value={candidate.profileKey} disabled={member.enabled && candidate.status !== 'compatible' && candidate.profileKey !== member.modelProfileKey}>{candidate.publicName} · {admissionLabel(candidate.status)}</option>)}</select></label><label><span>交接顺序</span><input type="number" min="1" max="100" value={member.fallbackPriority} disabled={busy} onChange={(e)=>void update(member.memberKey,{fallbackPriority:Number(e.target.value)},`已调整${member.displayName}的交接顺序`)}/></label></div>
