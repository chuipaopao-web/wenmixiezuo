...
export function CreationOperationsPage(): React.JSX.Element {
  ...
  return <section className="creation-ops-page">
    ...
    </details>)}{tasks.length === 0 && planningTasks.length === 0 ? <div className="creation-ops-state"><ClockCounterClockwise/><strong>还没有创作任务</strong></div> : tasks.map((task) => <details key={task.workflowId} onToggle={(event) => { if (event.currentTarget.open) void loadAudit(task); }}>
