...
export function RhythmAssetsPage(): React.JSX.Element {
  ...
  const toggle = (key: string): void => { const current = draft.layers[layer]; setDraft({ ...draft, layers: { ...draft.layers,
    [layer]: current.includes(key) ? current.filter(k => k !== key) : [...current, key],
    ...(layer === 'book_backbone' ? {volume_distribution:current.includes(key) ? current.filter(k=>k!==key) : [...current,key]} : {}) } }); setNotice(''); };
  ...
  return <section className="rhythm-page">
    <header className="rhythm-heading"><div><h2>分层方法库</h2><p>名称＋简短介绍。各层分别选用，同一方法可以跨层使用。</p></div><span>当前 v{saved.version} · {saved.enabled ? '运行供给已启用' : '运行供给未启用'}</span></header>
    {error && <p role="alert" className="rhythm-error">{error}</p>}{notice && <p role="status">{notice}</p>}
    <nav className="rhythm-tabs" aria-label="节奏供给层">{Object.entries(RHYTHM_LAYERS).filter(([key])=>key!=='volume_distribution').map(([key, value]) => <button key={key} aria-pressed={layer === key} onClick={() => setLayer(key as PlanningLayerKey)}>{value.label}</button>)}</nav>
    <p>{RHYTHM_LAYERS[layer].responsibility}</p>
    <div className="rhythm-columns"><section className="rhythm-panel"><h3>本层可用 · {draft.layers[layer].length}项</h3><p>勾选后提供给本层成员，成员按需搭配；取消勾选即可停用，历史任务保留原版。</p>
      {Object.entries(RHYTHM_CATEGORIES).map(([key, label]) => <div key={key} className="rhythm-category"><h4>{label}</h4>{draft.cards.filter(c => c.category === key).map(c => <div className="rhythm-card-row" key={c.key}>
        ...
        <button aria-pressed={card.key === c.key} onClick={() => setSelected(c.key)}><strong>{c.title}</strong><span style={{display:'block',fontWeight:400}}>{c.instruction}</span></button>
      </div>)}</div>)}
      ...
      <label>简短介绍<textarea value={card.instruction} maxLength={120} onChange={e => patchCard({ instruction: e.target.value })} /></label><small>{card.instruction.length}/120字符</small></fieldset>
    </section></div>
