...
it('显示分层候选预览，修改发布后显示真实版本',async()=>{
  ...
  render(<RhythmAssetsPage/>); await screen.findByText('本层可用 · 11项');
  fireEvent.click(screen.getByRole('button',{name:'章'})); expect(screen.getByText('本层可用 · 6项')).toBeVisible();
...
it('读取失败可重试，允许结构跨层使用，发布失败保留修改',async()=>{
  ...
  await screen.findByText('本层可用 · 11项'); fireEvent.click(screen.getByRole('button',{name:'章'}));
  fireEvent.click(screen.getByLabelText('提供单主线推进')); expect(screen.getByRole('button',{name:'发布共用配置'})).not.toBeDisabled();
