...
it('显示分层候选预览，修改发布后显示真实版本',async()=>{
  ...
  render(<RhythmAssetsPage/>); await screen.findByText('本层候选 · 5/6');
  fireEvent.click(screen.getByRole('button',{name:'章'})); expect(screen.getByText('本层候选 · 3/6')).toBeVisible();
  fireEvent.change(screen.getByLabelText('给 AI 的短指令'),{target:{value:'一项长期目标推进，每次成果改变局势。'}});
...
it('读取失败可重试，非法跨层禁止发布，发布失败保留修改',async()=>{
  ...
  await screen.findByText('本层候选 · 5/6'); fireEvent.click(screen.getByRole('button',{name:'章'}));
  fireEvent.click(screen.getByLabelText('提供单主线推进')); expect(screen.getByRole('button',{name:'发布共用配置'})).toBeDisabled();
  fireEvent.click(screen.getByLabelText('提供单主线推进')); fireEvent.change(screen.getByLabelText('名称'),{target:{value:'新的名称'}});
