...
export const RHYTHM_FRAGMENT_LIMIT = 2400;
...
export function validateRhythmPolicy(value: unknown): RhythmPolicy {
  ...
  for (const layer of Object.keys(RHYTHM_LAYERS) as PlanningLayerKey[]) {
    ...
    if (!Array.isArray(selected) || selected.length < 1 || selected.length > 24 || new Set(selected).size !== selected.length
      || selected.some(key => !keys.has(key))) throw new Error(`${RHYTHM_LAYERS[layer].label}请选择1～24个不同方法，整体片段仍需符合预算。`);
