...
const WORKSTATIONS: ReadonlyArray<readonly [V7WorkstationKey, string, readonly V7AgentTaskKind[], string, readonly string[], readonly string[], readonly string[], string]> = [
  ...
  ['chain', '单元链', ['planning_context', 'planning_recipe', 'planning_tree', 'planning_review'], '设计符合本书目标的因果推进、人物选择、情绪与待兑现的承诺。', ['当前卷方案', '当前链前置状态', '相关人物事实', '少量剧情候选'], ['无关卷资料', '整套剧情库', '未确认未来事实'], ['因果与人物动机可信', '节奏和回报时机符合作者目标与本书题材', '已兑现与留待后续的承诺区分清楚', '不靠重复冲突或强塞转折制造推进'], '只设计当前链，不替正文宣告实际结果；不设置通用章节数、固定回报间隔或每链必须变化的门槛。作者明确选择的具体目标仍须遵守。'],
  ['chapter_outline', '章纲', ['chapter_outline', 'chapter_outline_review'], '把当前链责任变成可以直接写的章纲，并核对承接、人物动机与本章的叙事作用。', ['当前链', '相关人物与事实', '上一章落点', '待审章纲'], ['无关全书资料', '其他候选章纲'], ['场景具体可写', '人物行为有动机', '铺垫、氛围、交流或事件推进服务本章目标', '节奏符合本书需要，不强制章末转折或状态变化', '审查者与设计者模型不同'], '章纲表达本章目标并保留正文发挥空间；审查不直接覆盖候选，不以通用节拍代替作者选择。'],
  ['manuscript', '正文', ['manuscript'], '依据确认章纲与正式资料写出自然、具体、有作者声音的完整正文。', ['确认章纲', '最小人物与事实资料', '相邻正文'], ['未确认未来计划', '完整方法库', '其他主笔草稿'], ['不违背既有事实和作者硬要求', '人物声音与变化有行为依据', '语言自然', '本章叙事作用清楚'], '保留作者硬要求和既有事实；开放处允许新细节、合理人物选择与发现，不把规划建议等同已发生事实。与硬要求冲突时说明问题，不擅自覆盖正式资料。'],
...
const SKILLS: ReadonlyArray<readonly [string, string, readonly V7AgentTaskKind[], readonly string[], readonly string[], readonly string[]]> = [
  ...
  ['evidence-review', '独立证据审查', ['opening_review', 'setting_review', 'planning_review', 'chapter_outline_review', 'manuscript_review'], ['逐条引用当前版本证据', '区分事实错误、风险与文学偏好；悬念、有限视角和合理人物变化不自动判错', '修改范围与实际影响相称，保留已有优秀表达', '只有作者明确选择的节奏目标才作相应检查，不因偏离通用模板要求重写', '需要作者决定时只保留真正分歧'], ['正式资料读取', '候选或正文读取', '证据引用'], ['审查者与生成者模型相同且任务要求独立', '证据版本不匹配']],
  ['natural-prose', '自然正文创作', ['manuscript'], ['先明确场景目标和人物动机', '按叙述需要选择动作、对话、感官细节或概述，不把表达手段逐项堆满', '控制解释密度，避免重复总结与机械句式', '结尾服务本章作用，允许自然停顿、余韵或悬念，不强制变化与转折'], ['章纲读取', '相关正文检索'], ['作者硬要求或既有事实互相冲突', '缺少影响当前写作的关键人物事实']],
