...
describe('V7 author opening flow', () => {
  ...
  it('keeps the main navigation visible while the bookshelf drawer opens and closes', async () => {
    installFetch();
    render(<AuthorApp />);
    ...
    expect(screen.getByRole('button', { name: '打开书架' })).toHaveAttribute('aria-expanded', 'true');
    ...
    expect(document.activeElement).toBe(screen.getByRole('button', { name: '打开书架' }));
    await waitFor(() => expect(fetch).toHaveBeenCalled());
  });
