...
import {
  ...
  fetchCurrentSettingFinalReview, fetchCurrentSettingRecommendation, fetchSettingDepartment, fuseSettingItem, redesignSettingItem, retrySettingBatch, retrySettingFinalReview, retrySettingRecommendation,
  type SettingBatchView, type SettingCatalogRecommendationView, type SettingDepartmentView, type SettingFinalReviewView, type SettingItemView, type SettingRedesignCandidate
} from './opening-api';
...
export function SettingPage({ bookId, onOpenTimeMachine }: { bookId: string; onOpenTimeMachine?: () => void }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void fetchSettingBatch(bookId, batch.batchId).then((next) => {
      setBatch(next);
      if (!['queued', 'working'].includes(next.status)) void load().catch((reason: unknown) => setError(message(reason)));
    }).catch((reason: unknown) => setError(message(reason))); }, 1_200);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void fetchCurrentSettingRecommendation(bookId).then((next) => {
    ...
    return () => window.clearInterval(timer);
  }, [bookId, department?.confirmedItems, department?.recommendation]);
  useEffect(() => {
    if (finalReview === null || !['queued', 'working'].includes(finalReview.status)) return;
    const timer = window.setInterval(() => { void fetchCurrentSettingFinalReview(bookId).then((next) => {
      setFinalReview(next);
      if (next.status === 'ready') void load().then(() => setFinalReviewOpen(true)).catch((reason: unknown) => setError(message(reason)));
    }).catch((reason: unknown) => setError(message(reason))); }, 1_200);
    return () => window.clearInterval(timer);
  ...
  return (
    <section className="setting-page" aria-labelledby="setting-title">
      ...
      {showCatalog && <section className="setting-catalog-card" aria-labelledby="catalog-title">
        ...
        <div className="setting-catalog-actions"><small>新增条目默认每项只生成一份，避免多人重复读取同一资料；完成后可在单条结果中选择一至三位编剧重新设计。</small><button type="button" className="primary-action" disabled={busy || selectableCount === 0} onClick={() => void start()}><SparkleIcon />{busy ? '成员正在设计…' : `设计新增${selectableCount}项`}</button></div>
      </section>}

      {batch && <EditorialRoom batch={batch} overallProgress={overallProgress} onRetry={() => void retrySettingBatch(bookId, batch.batchId).then(setBatch).catch((reason: unknown) => setError(message(reason)))}/>} 
      ...
      {items.length > 0 && <section className={`setting-results ${showResults ? 'expanded' : 'collapsed'}`} aria-labelledby="setting-results-title">
        ...
        {showResults && items.map((item) => <SettingResultCard
          ...
          onApplied={(next) => { updateItem(next, setBatch, setDepartment); setEditing(null); setRedesigning(null); }}
          onAdoptChief={async () => {
      ...
      {items.length > 0 && <section className="setting-finish-panel" aria-label="设定收尾">
        <div><strong>{allDesigned ? '本轮设定已经设计完成' : '完成全部条目后，由主编统一整理'}</strong><small>{allDesigned ? `共 ${items.length} 项，还会跨条目统一核对名称、时间与规则。` : `当前完成 ${overallProgress.completed}/${overallProgress.total} 项。`}</small></div>
        <button type="button" className="secondary-action" disabled={!allDesigned || finalReviewBusy} onClick={() => void beginFinalReview()}><ClipboardTextIcon />{finalReviewBusy ? '正在请主编接单…' : finalReview?.status === 'ready' ? '查看统一整理结果' : finalReview?.status === 'queued' || finalReview?.status === 'working' ? '主编正在统一整理' : '请主编统一整理'}</button>
      </section>}
      {finalReviewOpen && allDesigned && finalReview !== null && <section className={`setting-final-review ${finalReview.status}`} aria-label="主编统一整理结果">
        ...
        {finalReview.status === 'failed' && <div className="error-notice"><span>{publicFailureCopy(finalReview.statusText)}</span>{finalReview.retryable && <button type="button" disabled={finalReviewBusy} onClick={() => void continueFinalReview()}>{finalReviewBusy ? '正在继续…' : '继续统一整理'}</button>}</div>}
        ...
        {finalReview.status === 'ready' && <><footer><button type="button" className="primary-action" disabled={savingAll || pendingConfirmation.length === 0} onClick={() => {
        ...
        }}><CheckCircleIcon />{savingAll ? '正在逐项保存…' : pendingConfirmation.length === 0 || finalSaved ? '当前设定已保存' : `保存当前设定（${pendingConfirmation.length}项）`}</button></footer>
        {(finalSaved || pendingConfirmation.length === 0) && <div className="setting-next-step"><span><strong>下一步：时光机</strong><small>设定已经安全保存，可以查看全书框架。</small></span><button type="button" disabled={onOpenTimeMachine === undefined} onClick={onOpenTimeMachine}>进入时光机</button></div>}</>}
      </section>}
...
function SettingRecommendationPanel({
  ...
  if (recommendation === null) return <section className="setting-recommendation-card ready-to-start" aria-label="主编整理设定清单">
    ...
    <button type="button" className="primary-action" disabled={busy} onClick={onStart}><SparkleIcon />{busy ? '正在下单…' : '请主编整理设定清单'}</button>
  </section>;
  ...
  if (recommendation.status === 'failed') return <section className="setting-recommendation-card failed" role="alert">
    ...
    {recommendation.retryable
      ...
      : <button type="button" className="primary-action" disabled={busy} onClick={onStart}><SparkleIcon />{busy ? '正在重新整理…' : '按最新要求重新整理'}</button>}
  </section>;
...
function EditorialRoom({ batch, overallProgress, onRetry }: { batch: SettingBatchView; overallProgress: { completed: number; total: number }; onRetry: () => void }): React.JSX.Element {
...
interface SettingResultCardProps {
  ...
  onApplied: (item: SettingItemView) => void;
  onAdoptChief: () => void;
  onConfirm: () => void;
}
...
function SettingResultCard(props: SettingResultCardProps): React.JSX.Element {
  ...
  return <article className={`setting-result-card ${item.state} ${expanded ? 'expanded' : 'collapsed'}`}>
    ...
    {expanded && <>
      ...
      {redesigning && <InlineRedesignPanel bookId={props.bookId} item={item} members={members.filter((member) => publicRoleLabel(member.role) === '策划编剧')} onClose={props.onCloseInline} onTaskStarted={props.onTaskStarted} onApplied={props.onApplied}/>} 
    </>}
  </article>;
}
...
function InlineEditPanel({ bookId, item, onClose, onTaskStarted }: { bookId: string; item: SettingItemView; onClose: () => void; onTaskStarted: (batch: SettingBatchView) => void }): React.JSX.Element {
  ...
  return <section className="setting-inline-panel setting-inline-edit" aria-label={`修改${item.label}`}>
    ...
    <textarea maxLength={2000} value={content} onChange={(event) => setContent(event.target.value)}/><span className="field-count">{Array.from(content).length}/2000</span>
...
function InlineRedesignPanel({ bookId, item, members, onClose, onTaskStarted, onApplied }: { bookId: string; item: SettingItemView; members: SettingBatchView['members']; onClose: () => void; onTaskStarted: (batch: SettingBatchView) => void; onApplied: (item: SettingItemView) => void }): React.JSX.Element {
  ...
  const run = async (): Promise<void> => {
    setBusy(true); setError(null);
    try { const value = await redesignSettingItem(bookId, item.itemKey, selected, note); setCandidates(value.candidates); setFailedMemberKeys(value.failedMemberKeys ?? []); setChosen(value.candidates.slice(0, 1).map((candidate) => candidate.outputId)); }
    catch (reason) { setError(message(reason)); }
  ...
  const apply = async (): Promise<void> => {
    ...
    try {
      if (chosen.length === 1) {
        ...
        onApplied(await fuseSettingItem(bookId, item.itemKey, chosen, note));
      }
  ...
  return <section className="setting-inline-panel redesign-panel" aria-label={`重新设计${item.label}`}>
    <header><span><strong>重新设计 · {item.label}</strong><small>{candidates.length === 0 ? '默认一位强模型成员；需要比较时再加到三位，每份结果独立保存。' : '勾选一份采用，或勾选多份交给主编融合。'}</small></span><button type="button" aria-label="收起重新设计" onClick={onClose}><XIcon /></button></header>
    {candidates.length === 0 ? <>
      <div className="redesign-members">{availableMembers.map((member) => <label key={member.memberKey} className={selected.includes(member.memberKey) ? 'selected' : ''}>
        <input type="checkbox" disabled={member.presence === 'leave'} checked={selected.includes(member.memberKey)} onChange={(event) => setSelected((current) => event.target.checked ? current.length < 3 ? [...current, member.memberKey] : current : current.filter((key) => key !== member.memberKey))}/>
      ...
      <textarea maxLength={800} placeholder="补充您的调整意见（可不填）" value={note} onChange={(event) => setNote(event.target.value)}/><span className="field-count">{Array.from(note).length}/800</span>
      <footer><button type="button" onClick={onClose}>取消</button><button type="button" className="primary-action" disabled={busy || selected.length < 1} onClick={() => void run()}>{busy ? '编剧正在设计…' : `请${selected.length}名编剧出方案`}</button></footer>
    </> : <>
      ...
      <footer><button type="button" onClick={() => setCandidates([])}>重新选择成员</button><button type="button" className="primary-action" disabled={busy || chosen.length < 1} onClick={() => void apply()}>{busy ? '正在整理…' : chosen.length > 1 ? `融合${chosen.length}份方案` : '采用并交主编复审'}</button></footer>
    </>}
    {failedMemberKeys.length > 0 && <div className="error-notice">抱歉，有{failedMemberKeys.length}位编剧这次没有完成；已完成方案都已保留，您仍可直接采用。</div>}
    {error && <div className="error-notice">{error}</div>}
  </section>;
}
...
function message(reason: unknown): string {
  if (reason instanceof AuthorApiError && reason.status === 409) return reason.message;
